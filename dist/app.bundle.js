...
const getJsonPathValue = (root, path = "") => {
  ...
  return parts.reduce((current, part) => current && typeof current === "object" ? current[part] : undefined, root);
};
    ...
    listeners.forEach((listener) => {
      ...
      currentEntries.forEach(([key, value]) => {
        ...
        if (listener.event === eventName)
          listener.callback(createLocalSnapshot(key, value));
      });
    ...
    put: (file) => {
      const task = {
        ...
        on: (_event, _progress, onError, onComplete) => {
          readBrowserFileAsDataUrl(file).then((url) => {
...
const getDefaultBattlePhotoPreferences = () => BATTLE_PHOTO_SLOTS.reduce((acc, slot) => {
  acc[slot.id] = "";
  return acc;
}, {});
...
const getNextPlayableIndex = (currentIndex, items, useRandom = false) => {
  ...
  if (total === 1)
    return 0;
  if (useRandom) {
    const candidateIndexes = items.map((item, idx) => ({ item, idx })).filter(({ item, idx }) => idx !== currentIndex && item && item.url).map(({ idx }) => idx);
...
const getGalleryFilterButtonStyle = (label, isActive = false) => {
  const styleForLabel = label === "INICIAL" ? GALLERY_LABEL_STYLES.DEFAULT : label === "100" ? GALLERY_LABEL_STYLES.B : getGalleryLabelStyle(label);
  return {
    "--btn-neon-color": styleForLabel.glow,
    borderColor: styleForLabel.border,
    color: styleForLabel.text,
    boxShadow: isActive ? `inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${styleForLabel.border}, 0 0 20px ${styleForLabel.glow}` : `inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${styleForLabel.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${styleForLabel.glow} 60%, transparent)`,
...
const getSafeImageSrc = (rawUrl = "", fallback = "") => {
  const normalized = String(rawUrl || "").trim();
  if (!normalized || isBlockedMediaUrl(normalized))
    return fallback;
  return normalized;
};
...
const getSafeGalleryArray = (gallery, key, fallbackType = "image") => Array.isArray(gallery?.[key]) ? gallery[key].map((item) => normalizeGalleryItem(item, fallbackType)).filter((item) => item.url) : [];
...
const getGallerySourceCharacterId = (profile = {}) => {
  if (profile?.isAnonymousGallery || profile?.firebaseId === ANON_PROFILE_ID)
    return "anonimo";
...
const hasAllMainPhotosAssigned = (profile = {}) => {
  ...
  if (!profilePhotoUrl)
    return false;
  return BATTLE_PHOTO_SLOTS.filter((slot) => slot.id !== "perfil").every((slot) => Boolean(String(prefs?.[slot.id] || "").trim()));
};
const getMissingMainPhotoLabels = (profile = {}) => {
  ...
  BATTLE_PHOTO_SLOTS.filter((slot) => slot.id !== "perfil").forEach((slot) => {
...
const openMultimediaTab = (profile = null) => {
  ...
  const galleryItems = Array.isArray(profile?.galeria?.fotos) ? profile.galeria.fotos.map((item) => normalizeGalleryItem(item, "image")).filter((item) => item.url).map((item, sourceIndex) => ({
    ...item,
    sourceTag: "fotos",
    sourceIndex
  })) : [];
  ...
  const galleryImageByUrl = galleryItems.filter((item) => item.type === "image" && item.url).reduce((acc, item) => {
    acc[item.url] = item;
    return acc;
  }, {});
  const galleryHtml = galleryItems.length ? galleryItems.map((item, index) => `
                    <button
                        type="button"
                ...
                `).join("") : '<p class="text-slate-300">Sin contenido en galería.</p>';
  const brokenGalleryHtml = galleryPhotoItems.length ? galleryPhotoItems.map((item, index) => `
                    <button
                        type="button"
                ...
                `).join("") : '<p class="text-slate-300">Sin imágenes en galería.</p>';
  const slotHtml = BATTLE_PHOTO_SLOTS.map((slot) => {
    ...
    const assignedPhoto = assignedUrl ? isProfileSlot ? { url: assignedUrl } : galleryImageByUrl[assignedUrl] || { url: assignedUrl, label: "", sourceIndex: -1 } : null;
    ...
    return `
                    <div class="multimedia-slot-card ${isAssigned ? "is-assigned" : "is-missing"}" data-slot-id="${slot.id}">
                        ...
                        <div class="multimedia-slot-preview">
                            ${isAssigned ? `<img src="${assignedPhoto.url}" alt="${slot.label}" loading="lazy" />` : '<span class="multimedia-slot-empty">Sin foto designada</span>'}
                        </div>
...
const MultimediaModal = ({
  ...
  if (!isOpen || !profile)
    return null;
  const galleryItems = Array.isArray(profile?.galeria?.fotos) ? profile.galeria.fotos.map((item) => normalizeGalleryItem(item, "image")).filter((item) => item.url) : [];
  const topScores = Object.entries(profile?.puntuaciones || {}).map(([label, value]) => ({ label, value: Number(value) || 0 })).sort((a, b) => b.value - a.value).slice(0, 5);
  return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
    ...
    onClick: onClose,
    role: "presentation",
    children: /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
      ...
      children: [
        /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
          type: "button",
          onClick: onClose,
          className: "absolute top-4 right-4 w-10 h-10 rounded-full border border-cyan-200/35 bg-slate-900/80 text-slate-100",
          children: "✕"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
          className: "font-title text-center text-3xl md:text-4xl text-white uppercase tracking-wide",
          children: "Multimedia"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
          className: "text-center text-cyan-100/80 text-xs uppercase tracking-[0.2em] mt-2",
          children: profile?.nombre || "Personaje"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
          className: "mt-6 space-y-4",
          children: [
            /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
              className: "surface-panel rounded-2xl border border-cyan-200/20",
              children: [
                /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                  type: "button",
                  onClick: () => setIsGalleryOpen((prev) => !prev),
                  className: "w-full px-4 py-3 text-left font-black uppercase tracking-wide flex items-center justify-between",
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                      children: "Galería"
                    }, undefined, false, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                      children: isGalleryOpen ? "−" : "+"
                    }, undefined, false, undefined, this)
                  ]
                }, undefined, true, undefined, this),
                isGalleryOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  className: "px-4 pb-4 grid grid-cols-2 md:grid-cols-4 gap-3",
                  children: galleryItems.length ? galleryItems.map((item, idx) => /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "rounded-xl overflow-hidden border border-slate-600/40 bg-slate-900/40",
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                      src: item.url,
                      alt: `Multimedia ${idx + 1}`,
                      className: "w-full h-36 object-cover",
                      loading: "lazy"
                    }, undefined, false, undefined, this)
                  }, `${item.url}-${idx}`, false, undefined, this)) : /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                    className: "text-sm text-slate-300 col-span-full",
                    children: "Sin contenido en galería."
                  }, undefined, false, undefined, this)
                }, undefined, false, undefined, this)
              ]
            }, undefined, true, undefined, this),
            /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
              ...
              children: [
                /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                  type: "button",
                  onClick: () => setIsTopOpen((prev) => !prev),
                  className: "w-full px-4 py-3 text-left font-black uppercase tracking-wide flex items-center justify-between",
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                      children: "5 Principales"
                    }, undefined, false, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                      children: isTopOpen ? "−" : "+"
                    }, undefined, false, undefined, this)
                ...
                isTopOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  className: "px-4 pb-4 space-y-2",
                  children: topScores.length ? topScores.map((item) => /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "surface-panel rounded-xl px-3 py-2 flex items-center justify-between",
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                        className: "text-sm text-slate-200 uppercase",
                        children: item.label
                      }, undefined, false, undefined, this),
                  ...
                  }, item.label, true, undefined, this)) : /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                    className: "text-sm text-slate-300",
                    children: "Sin puntajes cargados."
                  }, undefined, false, undefined, this)
                }, undefined, false, undefined, this)
              ]
            }, undefined, true, undefined, this)
...
const getBattlePhotoForArena = (profile, arenaName) => {
  ...
  const galleryImages = Array.isArray(profile?.galeria?.fotos) ? profile.galeria.fotos.map((item) => normalizeGalleryItem(item, "image")).filter((item) => item.type === "image" && item.url) : [];
...
const getProfessionGlyph = (profession = "") => {
  ...
  if (lowP.includes("cantante"))
    return "crown";
  if (lowP.includes("nsfw"))
    return "torch";
  if (lowP.includes("actriz") || lowP.includes("actor"))
    return "theater";
  if (lowP.includes("modelo"))
    return "floron";
  if (lowP.includes("influencer"))
    return "sigil";
  if (lowP.includes("periodista"))
    return "quill";
  if (lowP.includes("bailarina"))
    return "floron";
  if (lowP.includes("atleta"))
    return "shield";
  return "shield";
};
...
const getProfessionCardVisual = (profession = "") => {
  ...
  const baseColor = String(config?.color || "rgba(148, 163, 184, 0.8)").replace(/,\s*[\d.]+\)$/, ", 1)");
  const glowColor = String(config?.color || "rgba(148, 163, 184, 0.8)").replace(/,\s*[\d.]+\)$/, ", 0.75)");
...
const HeraldicGlyph = ({ variant = "sigil", size = 18, className = "", tint = "currentColor" }) => {
  ...
  if (variant === "crown") {
    return /* @__PURE__ */ jsxDEV_7x81h0kn("svg", {
      ...common,
      stroke: tint,
      strokeWidth: "1.7",
      strokeLinecap: "round",
      ...
      children: [
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M3 17h18v3H3z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M4 17l3-8 5 5 5-7 3 10"
        }, undefined, false, undefined, this)
      ]
    }, undefined, true, undefined, this);
  }
  if (variant === "shield") {
    return /* @__PURE__ */ jsxDEV_7x81h0kn("svg", {
      ...
      children: [
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M12 3l7 3v6c0 4.3-2.4 7-7 9-4.6-2-7-4.7-7-9V6l7-3z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M9.5 11h5M12 8.5v5"
        }, undefined, false, undefined, this)
      ]
    }, undefined, true, undefined, this);
  }
  if (variant === "floron") {
    return /* @__PURE__ */ jsxDEV_7x81h0kn("svg", {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M6 9c1.4 0 2.8.9 3.4 2.2C8.2 11.5 7 12.8 7 14.3 5.4 13.5 4.3 11.9 4.3 10 4.3 9.5 5 9 6 9z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M18 9c-1.4 0-2.8.9-3.4 2.2 1.2.3 2.4 1.6 2.4 3.1 1.6-.8 2.7-2.4 2.7-4.3 0-.5-.7-1-1.7-1z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("circle", {
          cx: "12",
          cy: "12",
          r: "2.2"
        }, undefined, false, undefined, this)
  ...
  if (variant === "quill") {
    return /* @__PURE__ */ jsxDEV_7x81h0kn("svg", {
      ...
      children: [
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M20 4c-7 1-11.5 5.2-13 12l4-1 1 4c6.8-2.4 8.4-8.4 8-15z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M8 16l-4 4"
        }, undefined, false, undefined, this)
  ...
  if (variant === "torch") {
    return /* @__PURE__ */ jsxDEV_7x81h0kn("svg", {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M8 14l2 2 2-2-2-2z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M11 16l-4 5h10l-4-5"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M15.5 4.5c.9 1.3.5 3.2-1 4.3-1.5-1.4-1.8-3.3-.8-4.9.6.1 1.2.3 1.8.6z"
        }, undefined, false, undefined, this)
      ]
    }, undefined, true, undefined, this);
  }
  if (variant === "theater") {
    return /* @__PURE__ */ jsxDEV_7x81h0kn("svg", {
      ...
      children: [
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M5 6h14v11a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2z"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M9 11h.01M15 11h.01"
        }, undefined, false, undefined, this),
        /* @__PURE__ */ jsxDEV_7x81h0kn("path", {
          d: "M8.5 15c1 .8 2.2 1.2 3.5 1.2s2.5-.4 3.5-1.2"
        }, undefined, false, undefined, this)
      ]
    }, undefined, true, undefined, this);
  }
...
const LucideIcon = ({ name, size = 20, className = "" }) => {
  return /* @__PURE__ */ jsxDEV_7x81h0kn("i", {
  ...
  targetWindow.document.write(`
    ...
    return `
                            <div class="gallery-slot-card" data-slot-id="${slot.id}" style="border:1px solid ${hasSelection ? "rgba(74,222,128,0.95)" : "rgba(248,113,113,0.95)"}; border-radius:10px; padding:10px; background:${hasSelection ? "linear-gradient(155deg, rgba(20,83,45,0.95) 0%, rgba(21,128,61,0.82) 45%, rgba(5,46,22,0.96) 100%)" : "linear-gradient(155deg, rgba(127,29,29,0.96) 0%, rgba(185,28,28,0.84) 45%, rgba(69,10,10,0.96) 100%)"}; box-shadow: inset 0 2px 0 rgba(255,255,255,0.24), inset 0 -2px 0 rgba(2,6,23,0.45), inset 0 0 0 1px ${hasSelection ? "rgba(187,247,208,0.25)" : "rgba(254,202,202,0.22)"}, 0 8px 16px rgba(2,6,23,0.45), 0 0 18px ${hasSelection ? "rgba(74,222,128,0.35)" : "rgba(248,113,113,0.3)"}; position: relative;">
                                ...
                                <div style="margin-top:8px; border-radius:8px; overflow:hidden; aspect-ratio:4/3; border:1px dashed rgba(148,163,184,0.45); background: rgba(2,6,23,0.86); display:flex; align-items:center; justify-content:center;">
                                    ${hasSelection ? `<img src="${assignedUrl}" alt="Vista previa ${slot.label}" onerror="${BROKEN_IMAGE_INLINE_HANDLER}" style="width:100%; height:100%; object-fit:cover; display:block;" />` : '<span style="font-size:10px; color:#fca5a5; text-transform:uppercase; letter-spacing:0.08em; font-weight:700; text-align:center; padding:0 8px;">Sin foto designada</span>'}
                                </div>
    ...
    return `
                            <div class="viewer-slide" id="viewer-slide-${index}">
                                ${itemType === "video" ? embedInfo ? `<iframe src="${embedInfo.src}" title="${embedInfo.title} ${index + 1}" style="width:min(92vw, 1400px); height:min(75vh, 820px); border:0; border-radius:24px; box-shadow:0 0 40px rgba(34, 211, 238, 0.2); background:#000;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>` : `<video src="${foto.url}" controls playsinline preload="metadata" style="max-width:min(92vw, 1400px); max-height:calc(100vh - 64px); width:auto; height:auto; border-radius:24px; box-shadow:0 0 40px rgba(34, 211, 238, 0.2); background:#000;"></video>` : `<img src="${foto.url}" alt="Vista completa ${index + 1}" onerror="${BROKEN_IMAGE_INLINE_HANDLER}" />`}
                            </div>`;
  }).join("")}
                    </div>
                </div>
...
const App = () => {
  ...
  const getNeonClassByProfession = (profession = "") => {
    const normalizedKey = String(profession || "").normalize("NFD").replace(/[̀-ͯ]/g, "").trim().toUpperCase();
  ...
  const mapProfileToFormData = (profile = {}) => {
    ...
    const normalizedScores = safeProfile?.puntuaciones && typeof safeProfile.puntuaciones === "object" ? { ...baseFormData.puntuaciones, ...safeProfile.puntuaciones } : baseFormData.puntuaciones;
    return {
      ...safeProfile,
      ...baseFormData,
  ...
  const profileCompletionRows = useMemo(() => {
    ...
    const withStatus = rows.map((row) => {
      ...
      const previewValue = row.key === "fotos.0" ? isComplete ? "SI" : "NO" : isComplete ? String(normalizedValue) : "Sin completar";
      return {
        ...row,
        isComplete,
  ...
  const updateGalleryItemLabel = async ({ profileId, sourceTag, sourceIndex, label }) => {
    if (!profileId || !sourceTag || !Number.isInteger(sourceIndex))
      return;
    const normalizedLabel = GALLERY_LABELS.includes(label) ? label : "";
    const galleryRef = db.ref(`perfiles/${profileId}/galeria/${sourceTag}`);
    const snapshot = await galleryRef.once("value");
    const currentItems = Array.isArray(snapshot.val()) ? snapshot.val() : [];
    if (!currentItems[sourceIndex])
      return;
  ...
  const withProfilePhotoSyncedToGallery = (prevState, profilePhotoUrlRaw) => {
    ...
    const nextGallery = alreadyExists ? currentGallery : [...currentGallery, { url: normalizedProfileUrl, label: "C", type: "image" }];
    return {
      ...prevState,
  ...
  const addAnonymousGalleryItem = async ({ url, label, autor = "", forcedTag = "" }) => {
    ...
    const isValidMedia = inferredType === "image" || VIDEO_FILE_REGEX.test(normalizedUrl) || Boolean(getVideoEmbedInfo(normalizedUrl));
    if (!isValidMedia) {
  ...
  const removeGalleryAudioTrack = async (indexToRemove) => {
    ...
    try {
      ...
      const snapshot = await audioRef.once("value");
      const currentAudios = Array.isArray(snapshot.val()) ? snapshot.val() : [];
      if (!currentAudios[indexToRemove])
        return;
      const removedTrack = {
  ...
  useEffect(() => {
    const handleMessage = async (event) => {
      if (event.data.type === "ADD_IMAGE") {
        ...
        if (!id)
          return;
        const galleryRef = id === ANON_PROFILE_ID ? db.ref(`${ANON_GALLERY_NODE_PATH}/${tag}`) : db.ref(`perfiles/${id}/galeria/${tag}`);
        const snapshot = await galleryRef.once("value");
      ...
      if (event.data.type === "SET_BATTLE_PHOTO_PREF") {
        ...
        const slotConfig = getBattleSlotById(slotId);
        if (!id || id === ANON_PROFILE_ID || !slotConfig || !Number.isInteger(index))
          return;
        const tag = mediaType === "video" ? "videos" : "fotos";
        ...
        if (!selectedItem.url || selectedItem.type !== "image")
          return;
      ...
      if (event.data.type === "SET_BATTLE_PHOTO_PREF_BY_URL") {
        ...
        const slotConfig = getBattleSlotById(slotId);
        const normalizedUrl = String(url || "").trim();
        if (!id || id === ANON_PROFILE_ID || !slotConfig || !normalizedUrl)
          return;
        if (mediaType === "video")
          return;
        const prefsRef = db.ref(`perfiles/${id}/batallaFotosPreferidas/${slotId}`);
      ...
      if (event.data.type === "CLEAR_BATTLE_PHOTO_PREF") {
        ...
        const slotConfig = getBattleSlotById(slotId);
        if (!id || id === ANON_PROFILE_ID || !slotConfig)
          return;
        const prefsRef = db.ref(`perfiles/${id}/batallaFotosPreferidas/${slotId}`);
        await prefsRef.set("");
        setFormData((prev) => ({
    ...
    anonGalleryRef.on("value", (snapshot) => {
      ...
      const audios = Array.isArray(anonGalleryData?.audios) ? anonGalleryData.audios.map((audio) => ({
    ...
    categoriasRef.on("value", (snapshot) => {
      ...
      const categoriasList = Array.isArray(categoriasData) ? categoriasData : Object.entries(categoriasData).map(([firebaseId, category]) => ({
        ...category || {},
        firebaseId
      }));
  ...
  const getCategoryScore = (profile = {}, categoryKey = "") => {
    if (categoryKey === "Rostro")
      return getRostroScore(profile);
    if (categoryKey === "Cuerpo")
      return getCuerpoScore(profile);
    if (categoryKey === "Actitud")
      return getActitudScore(profile);
    return 0;
  };
  ...
  const obtenerCategoriasDePerfil = (p) => {
    ...
    categorias.forEach((c) => {
      const r = c.reglas;
      let matches = true;
      if (r.nacionalidades?.length > 0 && !r.nacionalidades.some((n) => nac === n.toLowerCase()))
        matches = false;
      if (r.profesiones?.length > 0 && !r.profesiones.some((p) => prof === p.toLowerCase()))
        matches = false;
      if (r.ciudades?.length > 0 && !r.ciudades.some((ciu_regla) => ciu === ciu_regla.toLowerCase()))
        matches = false;
      if (r.edadMin && (edad === "?" || edad < parseInt(r.edadMin)))
        matches = false;
      if (r.edadMax && (edad === "?" || edad > parseInt(r.edadMax)))
        matches = false;
      if (r.caracteristica && r.umbral) {
        const score = getProfileScores(p)[r.caracteristica] || 0;
        const umbral = parseInt(r.umbral);
        if (r.operador === "Superior a" && score <= umbral)
          matches = false;
        if (r.operador === "Inferior a" && score >= umbral)
          matches = false;
      }
      if (matches)
        cats.push(c.firebaseId);
    });
    return cats;
  };
  ...
  const allGalleryMediaEntries = useMemo(() => {
    return (perfiles || []).flatMap((perfil) => {
      ...
      const galleryItems = [
        ...Array.isArray(perfil?.galeria?.fotos) ? perfil.galeria.fotos.map((item, sourceIndex) => ({ item, sourceTag: "fotos", sourceIndex, fallbackType: "image" })) : [],
        ...Array.isArray(perfil?.galeria?.gifs) ? perfil.galeria.gifs.map((item, sourceIndex) => ({ item, sourceTag: "gifs", sourceIndex, fallbackType: "image" })) : [],
        ...Array.isArray(perfil?.galeria?.videos) ? perfil.galeria.videos.map((item, sourceIndex) => ({ item, sourceTag: "videos", sourceIndex, fallbackType: "video" })) : []
      ];
  ...
  const galleryBuckets = useMemo(() => {
    ...
    if (galleryViewMode === "PERSONAJE") {
      ...
      }).filter((bucket) => bucket.photos.length > 0).sort((a, b) => a.nombre.localeCompare(b.nombre, "es", { sensitivity: "base" }));
    }
    if (galleryViewMode === "ETIQUETA") {
      return GALLERY_LABELS.map((label) => {
  ...
  const activeGalleryBucket = useMemo(() => {
    if (galleryViewMode === "GENERAL")
      return galleryBuckets[0] || null;
    if (!selectedGalleryBucket)
      return null;
  ...
  const filteredGalleryPhotos = useMemo(() => {
    return sourceGalleryPhotos.filter((photo) => galleryViewMode === "ETIQUETA" ? true : galleryFilterLabel === "INICIAL" ? photo.label !== "X" : galleryFilterLabel === "100" ? true : photo.label === galleryFilterLabel);
  ...
  const selectedGalleryPhoto = selectedGalleryIndex === null ? null : filteredGalleryPhotos[clampIndex(selectedGalleryIndex, filteredGalleryPhotos.length)] || null;
  ...
  useEffect(() => {
    ...
    if (!isGalleryPlaying || !selectedGalleryPhoto)
      return;
  ...
  useEffect(() => {
    if (!selectedGalleryPhoto || !pendingFullscreenRequestRef.current)
      return;
  ...
  const isMobileViewport = typeof window !== "undefined" ? window.matchMedia("(max-width: 1023px), (pointer: coarse)").matches : false;
  ...
  const toggleEtiquetaSelection = (label) => {
    if (!label)
      return;
    setSelectedTagLabels((current) => current.includes(label) ? current.filter((selected) => selected !== label) : [...current, label]);
    setSelectedGalleryIndex(null);
  };
  const downloadAllGalleryPhotosAsZip = async () => {
    ...
    const sanitizeFileName = (value = "") => String(value || "").normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[\/:*?"<>|]/g, "-").replace(/\s+/g, " ").trim();
  ...
  const saveSelectedGalleryLabel = async () => {
    if (!selectedGalleryPhoto?.profileId || !selectedGalleryPhoto?.sourceTag || !Number.isInteger(selectedGalleryPhoto?.sourceIndex))
      return;
  ...
  const galleryContextLabel = useMemo(() => {
    if (galleryViewMode === "PERSONAJE") {
      return selectedCharacterBuckets.length ? `${selectedCharacterBuckets.length} personaje${selectedCharacterBuckets.length > 1 ? "s" : ""} seleccionado${selectedCharacterBuckets.length > 1 ? "s" : ""}` : "Sin personajes seleccionados";
    }
    if (activeGalleryBucket && isGalleryBucketMode)
      return activeGalleryBucket.nombre;
  ...
  const getArenaOrderedProfileIds = (profiles = []) => [...profiles || []].filter((profile) => profile?.firebaseId && (profile?.nombre || "").trim()).sort((a, b) => (a.nombre || "").localeCompare(b.nombre || "", "es", { sensitivity: "base" })).map((profile) => profile.firebaseId);
  const getBattleScopeOptions = (profiles = [], scopeId = "GENERAL") => {
    const mode = String(scopeId || "GENERAL").trim().toUpperCase();
    const orderedProfiles = [...profiles || []].filter((profile) => profile?.firebaseId && (profile?.nombre || "").trim()).sort((a, b) => (a.nombre || "").localeCompare(b.nombre || "", "es", { sensitivity: "base" }));
    if (mode === "GENERAL") {
    ...
    if (mode === "EDADES") {
      ...
      orderedProfiles.forEach((profile) => {
        ...
        if (!Number.isFinite(age) || age < 18)
          return;
        if (age <= 28)
          ageBuckets["18-28"].ids.push(profile.firebaseId);
        else if (age <= 39)
          ageBuckets["29-39"].ids.push(profile.firebaseId);
        else
          ageBuckets["40+"].ids.push(profile.firebaseId);
      });
    ...
    return Object.values(groupedMap).sort((a, b) => a.label.localeCompare(b.label, "es", { sensitivity: "base" }));
  };
  const buildArenaParticipants = (profiles = [], mode = "GENERAL", selectedGroupKey = "") => {
    const baseProfiles = [...profiles || []].filter((profile) => profile?.firebaseId && (profile?.nombre || "").trim()).sort((a, b) => (a.nombre || "").localeCompare(b.nombre || "", "es", { sensitivity: "base" }));
    if (mode === "GENERAL") {
      const ids = baseProfiles.map((profile) => profile.firebaseId);
      return {
        groupedIds: ids.length > 1 ? [{ key: "general", label: "General", typeLabel: "General", ids }] : [],
        orderedIds: ids
      };
    }
    ...
    const groupedIds = groupOptions.filter((group) => {
      if (!normalizedSelectedGroupKey || normalizedSelectedGroupKey === "all")
        return true;
  ...
  const buildArenaDerivedState = (rawDirectMatchups = {}, orderedIds = []) => {
    ...
    Object.entries(rawDirectMatchups || {}).forEach(([rawKey, rawRecord]) => {
      ...
      if (allowedIds.size && (!allowedIds.has(winnerId) || !allowedIds.has(loserId)))
        return;
      ...
      if (!victoryGraphSets[winnerId])
        victoryGraphSets[winnerId] = new Set;
    ...
    graphNodes.forEach((originId) => {
      ...
      while (queue.length) {
        ...
        if (!nodeId || visited.has(nodeId) || nodeId === originId)
          continue;
        visited.add(nodeId);
    ...
    const victoryGraph = Object.fromEntries(Object.entries(victoryGraphSets).map(([winnerId, losers]) => [winnerId, [...losers]]));
  ...
  const normalizeArenaGlobalState = (arenaName, rawArenaGlobalState = {}) => {
    ...
    const derivedState = buildArenaDerivedState(rawArenaGlobalState?.directMatchups || rawArenaGlobalState?.matchups || {}, orderedIds);
    return {
      arenaName: globalKey,
      orderedIds,
      ...derivedState
  ...
  const findNextPendingPairInGroup = (groupIds = [], playedMatchups = {}) => {
    for (let i = 0;i < groupIds.length - 1; i++) {
      for (let j = i + 1;j < groupIds.length; j++) {
        if (!hasArenaPairBeenPlayed(playedMatchups, groupIds[i], groupIds[j]))
  ...
  const isArenaStateKeyForArena = (arenaStateKey = "", arenaName = "") => {
    ...
    if (!arenaStateKey || !normalizedArena)
      return false;
  ...
  const mergeDirectMatchupRecord = (previousRecord, nextRecord) => {
    ...
    if (!normalizedNext || normalizedNext.reason !== "direct")
      return previousRecord || null;
    const normalizedPrevious = normalizeMatchupRecord(previousRecord);
    if (!normalizedPrevious || normalizedPrevious.reason !== "direct")
  ...
  const getGlobalArenaDerivedState = (arenaName, sourceArenaBattleState = arenaBattleState) => {
    ...
    Object.entries(sourceArenaBattleState || {}).forEach(([arenaStateKey, state]) => {
      if (!isArenaStateKeyForArena(arenaStateKey, normalizedArena))
        return;
    ...
    const allProfileIds = [...perfiles || []].map((profile) => profile?.firebaseId).filter(Boolean).sort((a, b) => String(a).localeCompare(String(b), "es", { sensitivity: "base" }));
  ...
  const buildArenaScoreMapFromGlobalStats = (arenaName, sourceArenaBattleState = arenaBattleState) => {
    ...
    return [...perfiles || []].filter((profile) => profile?.firebaseId).reduce((acc, profile) => {
      const profileStats = globalStats[profile.firebaseId] || { wins: 0, battles: 0 };
      acc[profile.firebaseId] = profileStats.battles ? Math.round(profileStats.wins / profileStats.battles * 100) : 0;
      return acc;
    }, {});
  };
  ...
  const normalizeArenaState = (arenaName, arenaState, globalStateForArena = null) => {
    ...
    const currentPairIsValid = !!currentChampionId && !!currentChallengerId && currentChampionId !== currentChallengerId && !!currentGroup && !hasArenaPairBeenPlayed(globalMatchups, currentChampionId, currentChallengerId);
    const nextPendingPair = groupedIds.length ? findNextPendingPairByGroups(groupedIds, arenaState.matchups || {}, globalMatchups) : null;
    const nextPair = currentPairIsValid ? [currentChampionId, currentChallengerId] : nextPendingPair;
  ...
  useEffect(() => {
    ...
    Object.entries(arenaBattleState || {}).forEach(([arenaKey, state]) => {
      ...
      updates.push(db.ref(`arenaBattleState/${arenaKey}`).set(normalized).catch((error) => console.error("No se pudo normalizar el estado del arena:", error)));
    });
    if (!Object.keys(repairedStates).length)
      return;
  ...
  useEffect(() => {
    ...
    Object.entries(arenaBattleState || {}).forEach(([arenaKey, state]) => {
      ...
      if (!globalKey)
        return;
      if (!legacyDirectByArena[globalKey])
        legacyDirectByArena[globalKey] = {};
    ...
    Object.entries(legacyDirectByArena).forEach(([arenaName, mergedDirectMatchups]) => {
      const globalKey = getArenaGlobalKey(arenaName);
      if (!globalKey || arenaGlobalState?.[globalKey])
        return;
      ...
      updates.push(db.ref(`arenaGlobalState/${globalKey}`).set(normalizedGlobal).catch((error) => console.error("No se pudo migrar arenaGlobalState:", error)));
    });
  ...
  const initArenaBattle = (arenaName, scopeId = selectedBattleScope, groupKey = selectedBattleGroupKey) => {
    const orderedProfiles = [...perfiles].filter((p) => p?.firebaseId && (p?.nombre || "").trim()).sort((a, b) => (a.nombre || "").localeCompare(b.nombre || "", "es", { sensitivity: "base" }));
    ...
    const activeGroup = initialPair ? getGroupForPair(groupedIds, initialPair[0], initialPair[1]) : null;
    ...
    if (!arenaGlobalState?.[globalKey]) {
      ...
      db.ref(`arenaGlobalState/${globalKey}`).set(normalizedGlobal).catch((error) => console.error("No se pudo guardar arenaGlobalState inicial:", error));
    }
    db.ref(`arenaBattleState/${arenaKey}`).set(nextArenaState).catch((error) => console.error("No se pudo guardar el estado del arena:", error));
  };
  ...
  const registerBattleWinner = (arenaName, winnerId, scopeId = selectedBattleScope, groupKey = selectedBattleGroupKey) => {
    ...
    const groupedIds = Array.isArray(state.groupedIds) && state.groupedIds.length ? state.groupedIds : buildArenaParticipants(perfiles, state.mode || "GENERAL", state.groupKey || "all").groupedIds;
    const { championId, challengerId } = state;
    if (winnerId !== championId && winnerId !== challengerId)
      return;
    const loserId = winnerId === championId ? challengerId : championId;
    if (!winnerId || !loserId)
      return;
    if (hasArenaPairBeenPlayed(currentGlobalState?.matchups || {}, winnerId, loserId))
      return;
    const pairKey = getPairKey(winnerId, loserId);
    ...
    const findNextOpponentForWinner = (ids, currentWinnerId, playedMatchups) => {
      ...
      for (const contenderId of ids) {
        if (!contenderId || contenderId === currentWinnerId)
          continue;
    ...
    const nextPair = winnerNextOpponentId ? [winnerId, winnerNextOpponentId] : findNextPendingPairByGroups(groupedIds, state.matchups || {}, updatedMatchups);
    const activeGroup = getGroupForPair(groupedIds, nextPair?.[0], nextPair?.[1]);
    ...
    setPerfiles((prev) => prev.map((profile) => {
      if (!profile?.firebaseId || !Object.prototype.hasOwnProperty.call(globalScoreMap, profile.firebaseId))
        return profile;
  ...
  const brokenProfileIds = useMemo(() => {
    return new Set(brokenGalleryPhotos.map((photo) => String(photo?.profileId || "").trim()).filter(Boolean));
  }, [brokenGalleryPhotos]);
  const tallerProfiles = useMemo(() => {
    ...
    const normalizeProfession = (profile) => {
      ...
      return professionFields.flatMap((value) => {
    ...
    return [...perfiles || []].filter((profile) => {
      if (!hasCoreProfileData(profile))
        return false;
      if (showIncompleteMainPhotosOnly && hasAllMainPhotosAssigned(profile))
        return false;
      if (showBrokenPhotosOnly) {
        const profileId = String(profile?.firebaseId || profile?.id || "").trim();
        if (!profileId || !brokenProfileIds.has(profileId))
          return false;
      }
      if (!normalizedSearch)
        return true;
  ...
  const selectedTallerProfile = useMemo(() => tallerProfiles.find((profile) => profile?.firebaseId === selectedTallerProfileId) || null, [tallerProfiles, selectedTallerProfileId]);
  useEffect(() => {
  ...
  const getSortValue = (profile, key) => {
    ...
    if (key === "nombre")
      return (profile.nombre || "").toLowerCase();
    if (key === "nacionalidad")
      return (profile.nacionalidad || "").toLowerCase();
    if (key === "edad") {
  ...
  const getDisplayedRankingScore = (profile) => {
    ...
    if (sortBy === "Rostro")
      return getRostroScore(profile).toFixed(0);
    if (sortBy === "Cuerpo")
      return getCuerpoScore(profile).toFixed(0);
    if (sortBy === "Actitud")
      return getActitudScore(profile).toFixed(0);
  ...
  const getScoreBreakdownByCategory = (profileId, categoryKey) => {
    ...
    const profileNameById = new Map((perfiles || []).filter((profile) => profile?.firebaseId).map((profile) => [profile.firebaseId, profile.nombre || "Sin nombre"]));
    const getSortedNames = (idsSet) => [...idsSet].map((id) => profileNameById.get(id)).filter(Boolean).sort((a, b) => a.localeCompare(b, "es", { sensitivity: "base" }));
    return {
  ...
  const getItemBattleBreakdown = (profileId, arenaName) => {
    ...
    const battles = Object.entries(arenaMatchups).map(mapBattle).filter(Boolean).sort((a, b) => a.opponentName.localeCompare(b.opponentName, "es", { sensitivity: "base" }));
    return {
      arenaName: safeArenaName,
  ...
  const visibleRankingProfiles = useMemo(() => isFullRankingVisible ? sortedProfiles : sortedProfiles.slice(0, 10), [sortedProfiles, isFullRankingVisible]);
  return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
    className: "app-space-theme flex h-screen w-screen overflow-hidden bg-[#020617] stone-wall-surface relative",
    children: [
      isSidebarOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("aside", {
        className: "hud-frame hud-frame--panel w-64 shrink-0 theme-surface-card stone-wall-surface border-r theme-border-secondary flex flex-col p-6 z-20 shadow-xl overflow-y-auto relative",
        children: [
          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
            className: "corner-ember-glow corner-ember-glow--left corner-ember-glow--top",
            "aria-hidden": "true"
          }, undefined, false, undefined, this),
          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
            ...
            children: [
              /* @__PURE__ */ jsxDEV_7x81h0kn("h1", {
                className: "neon-sign neon-sign--cyan text-5xl font-black text-[var(--metal-gold)] italic tracking-tighter leading-none",
                children: "G2"
              }, undefined, false, undefined, this),
              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                className: "text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] mt-2",
                children: "Elite Database"
              }, undefined, false, undefined, this)
            ]
          }, undefined, true, undefined, this),
          /* @__PURE__ */ jsxDEV_7x81h0kn("nav", {
            className: "flex-1 space-y-2 mb-8",
            children: [
            ...
            ].map((item) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
              onClick: () => {
              ...
              children: [
                /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
                  name: item.icon,
                  size: 18
                }, undefined, false, undefined, this),
                item.label
      ...
      /* @__PURE__ */ jsxDEV_7x81h0kn("main", {
        className: "flex-1 min-w-0 w-full flex flex-col overflow-hidden bg-[#020617] stone-wall-surface relative",
        children: [
          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
            ...
            children: /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
              name: isSidebarOpen ? "panel-left-close" : "panel-left-open",
              size: 12
            }, undefined, false, undefined, this)
          }, undefined, false, undefined, this),
          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
            className: `hud-section flex-1 overflow-y-auto relative z-10 stone-wall-surface ${isSidebarOpen ? "p-6 md:p-10 lg:p-12" : "p-4 md:p-8 lg:p-10"}`,
            children: [
              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "corner-ember-glow corner-ember-glow--left corner-ember-glow--top",
                "aria-hidden": "true"
              ...
              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "corner-ember-glow corner-ember-glow--right corner-ember-glow--top",
                "aria-hidden": "true"
              }, undefined, false, undefined, this),
              selectedTallerProfile && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                ...
                children: /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      className: "grid grid-cols-1 lg:grid-cols-[minmax(260px,320px),1fr] gap-8 items-start",
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "taller-detail-avatar rounded-[1.8rem] overflow-hidden border border-cyan-200/35",
                          children: /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: "text-[10px] uppercase tracking-[0.35em] text-cyan-200/90 font-black",
                                  children: "Ficha del personaje"
                                }, undefined, false, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                                  className: "taller-detail-title text-3xl md:text-4xl font-black uppercase mt-2",
                                  children: selectedTallerProfile.nombre || "Sin nombre"
                                }, undefined, false, undefined, this)
                              ]
                            }, undefined, true, undefined, this),
                            !selectedTallerProfile?.isAnonymousGallery && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "grid grid-cols-1 gap-4",
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
                                  className: "taller-data-chip",
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                      children: "Profesión"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("strong", {
                                      children: selectedTallerProfile.profesion || "No definida"
                                    }, undefined, false, undefined, this)
                                  ]
                                }, undefined, true, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
                                  ...
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                      children: "Nacionalidad - Ciudad"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("strong", {
                                      children: `${selectedTallerProfile.nacionalidad || "No definida"} - ${selectedTallerProfile.ciudad || "No definida"}`
                                    }, undefined, false, undefined, this)
                                  ]
                                }, undefined, true, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
                                  ...
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                      children: "Fecha de nacimiento - Edad"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("strong", {
                                      children: `${selectedTallerProfile.fechaNacimiento || "No informado"} - ${calcularEdad(selectedTallerProfile.fechaNacimiento)} años`
                                    }, undefined, false, undefined, this)
                                  ]
                                }, undefined, true, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
                                  ...
                                  children: [
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("strong", {
                                      children: selectedTallerProfile.estaturaCm ? `${selectedTallerProfile.estaturaCm} cm` : "No informada"
                                    }, undefined, false, undefined, this)
                                  ]
                                }, undefined, true, undefined, this)
                              ]
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              ...
                              children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "grid grid-cols-2 sm:grid-cols-[1fr,1fr,auto,auto] gap-3 items-center",
                                children: [
                                  !selectedTallerProfile?.isAnonymousGallery && /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                    type: "button",
                                    onClick: () => {
                                  ...
                                  !selectedTallerProfile?.isAnonymousGallery && /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                        type: "button",
                                        onClick: () => {
              ...
              activeTab === "TALLER" && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "space-y-8 animate-in fade-in duration-500",
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "space-y-2",
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
                      className: "neon-sign neon-sign--cyan text-4xl font-black italic text-white uppercase tracking-tighter",
                      children: "Taller"
                    }, undefined, false, undefined, this)
                  }, undefined, false, undefined, this),
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "max-w-2xl",
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                  ...
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                        ...
                        children: [
                          "Fotos incompletas ",
                          showIncompleteMainPhotosOnly ? "(Activo)" : ""
                        ]
                      }, undefined, true, undefined, this),
                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                        type: "button",
                  ...
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: tallerProfiles.map((p) => {
                      ...
                      return /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                        type: "button",
                        onClick: () => {
                        ...
                        onContextMenu: (event) => {
                          if (!showIncompleteMainPhotosOnly || !missingLabels.length)
                            return;
                          event.preventDefault();
                          event.stopPropagation();
                          setTallerMissingPhotosTooltipProfileId((prev) => prev === (p.firebaseId || p.nombre) ? "" : p.firebaseId || p.nombre);
                        },
                        onTouchStart: () => {
                          if (!showIncompleteMainPhotosOnly || !missingLabels.length)
                            return;
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "w-full aspect-[4/5] rounded-xl overflow-hidden mb-4 bg-slate-900/70",
                            children: /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                            className: "text-lg font-black text-white uppercase tracking-wide truncate",
                            children: p.nombre || "Sin nombre"
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            className: "text-xs text-slate-300 truncate",
                            children: p.nacionalidad || "Nacionalidad no definida"
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            ...
                            children: p.profesion || "Profesión no definida"
                          }, undefined, false, undefined, this),
                          showMissingPhotosTooltip && /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            type: "button",
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "font-black uppercase tracking-wider text-red-200 mb-2",
                                children: "Faltan fotos:"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                className: "space-y-1 mb-2",
                                children: missingLabels.map((label) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                className: "inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200",
                                children: "Ir a galería ↗"
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this)
                        ]
                      }, p.firebaseId || p.nombre, true, undefined, this);
                    })
                  }, undefined, false, undefined, this),
                  !tallerProfiles.length && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "rounded-2xl border border-slate-500/30 bg-slate-900/50 px-6 py-8 text-center text-sm text-slate-300",
                    children: "No hay personajes que coincidan con la búsqueda."
                  }, undefined, false, undefined, this),
                  /* @__PURE__ */ jsxDEV_7x81h0kn(MultimediaModal, {
              ...
              activeTab === "CAMPEONAS" && !selectedCategory && (() => {
                ...
                const getTopThree = (items = []) => [...items].sort((a, b) => (Number(calcularPromedio(b)) || 0) - (Number(calcularPromedio(a)) || 0)).slice(0, 3);
                ...
                CARACTERISTICAS.forEach((item) => {
                  const top = [...safeProfiles].sort((a, b) => (Number(b?.puntuaciones?.[item]) || 0) - (Number(a?.puntuaciones?.[item]) || 0)).slice(0, 3);
                ...
                [["Cuerpo", ["Cuerpo", "Cola", "Pechos", "Cintura", "Piernas", "Estatura"]], ["Rostro", ["Rostro", "Ojos", "Boca", "Cabello"]], ["Actitud", ["Sensualidad", "Carisma", "Elegancia", "Dulzura", "Talento"]]].forEach(([label, keys]) => {
                  const top = [...safeProfiles].map((p) => ({
                    ...p,
                    __metaScore: keys.reduce((acc, key) => acc + (Number(p?.puntuaciones?.[key]) || 0), 0) / keys.length
                  })).sort((a, b) => (b.__metaScore || 0) - (a.__metaScore || 0)).slice(0, 3);
                ...
                const renderPodiumCard = (card, isGeneralSection = false) => /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
                  className: `champion-podium-card champion-podium-card--${isGeneralSection ? "general" : "section"} rounded-2xl p-5`,
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                      className: `champion-podium-title text-xs font-black uppercase tracking-[0.18em] mb-3 ${isGeneralSection ? "text-[var(--metal-gold)]" : "text-cyan-100"}`,
                      children: card.title
                    }, undefined, false, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("ol", {
                      className: "space-y-2",
                      children: card.top.length ? card.top.map((p, idx) => {
                        const championThumb = getSafeImageSrc(String(p?.fotos?.[0] || "").trim(), "");
                        const isChampion = idx === 0;
                        return /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                          className: `champion-rank-row champion-rank-row--${idx + 1} flex items-center justify-between gap-3 text-xs`,
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "flex items-center gap-2 min-w-0",
                              children: [
                                isChampion && championThumb && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  className: "flex flex-col items-center shrink-0 -mt-1",
                                  title: `Campeona de ${card.title}`,
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                      className: "text-[12px] leading-none -mb-0.5",
                                      "aria-hidden": "true",
                                      children: "\uD83D\uDC51"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                                      src: championThumb,
                                ...
                                /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                  className: `champion-rank-name font-black truncate ${isChampion ? "champion-rank-name--winner" : "text-slate-100"}`,
                                  children: [
                                    idx + 1,
                                    ". ",
                                    p.nombre || "Sin nombre"
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                              className: `champion-score font-black shrink-0 ${idx === 0 ? "champion-score--winner" : ""}`,
                              children: calcularPromedio(p)
                            }, undefined, false, undefined, this)
                          ]
                        }, `${card.id}-${p.firebaseId || p.nombre || idx}`, true, undefined, this);
                      }) : /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                ...
                return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  ...
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
                          className: "neon-sign neon-sign--magenta text-4xl font-black italic text-white uppercase tracking-tighter",
                          children: "Campeonas"
                        }, undefined, false, undefined, this),
                        /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                          className: "text-xs font-bold text-[var(--metal-gold)] uppercase tracking-widest mt-1",
                          children: "Podios organizados por secciones"
                        }, undefined, false, undefined, this)
                      ]
                    }, undefined, true, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      className: "space-y-7",
                      children: sections.map((section) => /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                        className: "space-y-3",
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                            className: "text-[11px] font-black uppercase tracking-[0.2em] text-cyan-100/85",
                            children: section.title
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6",
                            children: section.cards.map((card) => renderPodiumCard(card, section.id === "general"))
                          }, undefined, false, undefined, this)
                        ]
              ...
              activeTab === "anonimo" && !selectedCategory && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "space-y-8 animate-in fade-in duration-500",
                children: /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                  className: "hud-frame hud-frame--panel profession-banner p-8 rounded-2xl gothic-frame gothic-frame--ornate gothic-frame--grand",
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                      className: "text-[11px] uppercase tracking-[0.35em] text-cyan-200/80 font-black",
                      children: "Modo"
                    }, undefined, false, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
                      className: "neon-sign neon-sign--magenta font-title text-5xl md:text-6xl font-black italic text-white tracking-[0.08em] leading-none mt-3",
                      children: "ánonimo"
                    }, undefined, false, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                      className: "text-sm md:text-base text-slate-200/85 mt-4 max-w-3xl",
                      children: "Este panel reutiliza el estilo visual del sistema y no abre la ficha de personaje para mantener una navegación discreta."
                    }, undefined, false, undefined, this),
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      ...
                      children: [
                        ...
                        (anonUploadType === "imagen" || anonUploadType === "escena") && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                              ...
                              children: GALLERY_LABELS.map((label) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                value: label,
                                children: label
                            ...
                            anonMediaError && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "md:col-span-2 text-xs font-black uppercase tracking-[0.12em] text-rose-300",
                              children: anonMediaError
                            }, undefined, false, undefined, this)
                          ]
                        }, undefined, true, undefined, this),
                        anonUploadType === "audio" && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "border-t border-cyan-300/25 pt-4 mt-2 space-y-3",
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-[11px] uppercase tracking-[0.25em] text-cyan-200/80 font-black",
                              children: "Audio para botón \uD83C\uDFB5"
                            }, undefined, false, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                              placeholder: "Nombre",
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              ...
                              children: "Guardar audio en JSON local"
                            }, undefined, false, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              ...
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: "text-[11px] uppercase tracking-[0.25em] text-cyan-200/80 font-black",
                                  children: "Audios guardados"
                                }, undefined, false, undefined, this),
                                galleryAudioTracks.length ? /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                  className: "space-y-2",
                                  children: galleryAudioTracks.map((track, index) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                                    className: "flex items-center justify-between gap-3 rounded-xl border border-slate-500/45 bg-slate-900/55 px-3 py-2",
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        className: "text-sm text-slate-100 font-bold truncate",
                                        children: track.nombre
                                      }, undefined, false, undefined, this),
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                        type: "button",
                                  ...
                                  }, `${track.url}-${track.nombre}-${index}`, true, undefined, this))
                                }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: "text-xs text-slate-300/80 font-semibold",
                                  children: "No hay audios guardados todavía."
                                }, undefined, false, undefined, this)
                              ]
                            }, undefined, true, undefined, this),
                            galleryAudioError ? /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-xs font-black uppercase tracking-[0.12em] text-rose-300",
                              children: galleryAudioError
              ...
              activeTab === "EXPLORAR" && !selectedCategory && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                ...
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    children: [
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                        className: "text-xs font-bold text-[var(--metal-gold)] uppercase tracking-widest mt-1",
                        children: "Organización automática por profesión"
                      }, undefined, false, undefined, this)
                    ]
                  }, undefined, true, undefined, this),
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8",
                    children: [...new Set((perfiles || []).map((p) => String(p?.profesion || "").trim()).filter(Boolean))].map((prof) => {
                      ...
                      return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "profession-folder-card__icon w-24 h-24 rounded-[2rem] flex items-center justify-center text-5xl mx-auto mb-6",
                            children: getEmoji(prof)
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                            className: "text-2xl font-black text-white mb-2 truncate uppercase tracking-tighter italic",
                            children: prof
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                              className: "text-[10px] font-black uppercase text-[var(--metal-gold)] tracking-widest",
                              children: [
                                count,
                                " ",
                      ...
                      }, prof, true, undefined, this);
                    })
                  }, undefined, false, undefined, this)
                ]
              }, undefined, true, undefined, this),
              selectedCategory && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                ...
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    style: {
                      "--banner-color": activeTab === "CATEGORIAS" ? "rgba(148, 163, 184, 0.8)" : getProfessionCardVisual(selectedCategory).baseColor
                    },
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            ...
                            title: "Volver",
                            children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "back-btn-silver p-2 rounded-xl text-slate-400 group-hover:text-slate-200 transition-all",
                              children: /* @__PURE__ */ jsxDEV_7x81h0kn("i", {
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
                        className: "neon-sign neon-sign--magenta font-title text-6xl font-black italic text-white tracking-[0.08em] leading-none",
                        children: activeTab === "CATEGORIAS" ? categorias.find((c) => c.firebaseId === selectedCategory)?.label || "Archivo del Reino" : selectedCategory
                      }, undefined, false, undefined, this)
                    ]
                  }, undefined, true, undefined, this),
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10",
                    children: (perfiles || []).filter((p) => {
                      if (!p)
                        return false;
                      ...
                      const scoreTierClass = scoreValue >= 95 ? "card-score-badge--gold" : scoreValue >= 86 ? "card-score-badge--diamond" : scoreValue >= 75 ? "card-score-badge--fire" : "";
                      const neonClass = getNeonClassByProfession(p.profesion);
                      return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        onClick: () => {
                          setContextMenuProfileId(null);
                        ...
                        children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "aspect-[4/5] bg-slate-950 relative overflow-hidden",
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              ...
                              children: [
                                ...
                                contextMenuProfileId === p.firebaseId && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  className: "absolute top-12 left-0 min-w-[170px] rounded-xl border theme-border-secondary bg-slate-950/95 shadow-2xl p-2 space-y-1",
                                  onClick: (event) => event.stopPropagation(),
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                      type: "button",
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              ...
                              children: [
                                scoreValue >= 75 && scoreValue <= 85 && /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                  className: "card-score-badge__fire-emoji",
                                  "aria-hidden": "true",
                                  children: "\uD83D\uDD25"
                                }, undefined, false, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                  className: "text-[9px] font-black text-[var(--metal-gold)] leading-none",
                                  children: "G2"
                                }, undefined, false, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                  className: "card-score-badge__value text-lg font-black",
                                  children: typeof calcularPromedio === "function" ? calcularPromedio(p) : "8.5"
                                }, undefined, false, undefined, this)
                              ]
                            }, undefined, true, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              ...
                              children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "text-bubble card-footer-profession w-full backdrop-blur-md p-4 rounded-2xl border",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                                    className: "text-lg font-black text-white italic tracking-tighter leading-none flex items-center gap-2",
                                    children: [
                                      p.nombre,
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        className: "text-[var(--metal-gold)] text-sm not-italic",
                                        children: [
                                          "(",
                                  ...
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                    ...
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        className: "text-[var(--metal-gold)] text-[7px] font-black uppercase tracking-widest",
                                        children: p.profesion
                                      }, undefined, false, undefined, this),
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                        className: "text-[7px] font-bold text-slate-400 uppercase",
                                        children: p.nacionalidad
                                      }, undefined, false, undefined, this)
                                    ]
                                  }, undefined, true, undefined, this)
                                ]
                      ...
                      }, p.firebaseId || Math.random(), false, undefined, this);
                    })
                  }, undefined, false, undefined, this),
                  contextMenuOpen && contextProfile && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ref: contextMenuRef,
              ...
              activeTab === "GALERIA" && !selectedCategory && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "space-y-10 animate-in fade-in duration-500",
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        children: [
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            className: "text-xs font-bold text-[var(--metal-gold)] uppercase tracking-widest mt-1",
                            children: `Vista por ${currentGalleryModeLabel} · ${galleryContextLabel}`
                          }, undefined, false, undefined, this)
                        ]
                      }, undefined, true, undefined, this),
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          GALLERY_VIEW_MODES.map((mode) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            type: "button",
                            ...
                            children: GALLERY_VIEW_MODE_LABELS[mode]
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "theme-surface-soft gothic-frame gothic-frame--secondary rounded-[1.8rem] px-5 py-4",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-[9px] font-black uppercase tracking-[0.25em] text-slate-500",
                                children: "Total"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-2xl font-black italic text-white mt-1",
                                children: sourceGalleryPhotos.length
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-2xl font-black italic text-[var(--metal-gold)] mt-1",
                                children: filteredGalleryPhotos.length
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-2xl font-black italic text-white mt-1",
                                children: GALLERY_LABELS.length
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  ...
                  galleryViewMode !== "PERSONAJE" && isGalleryBucketMode && !activeGalleryBucket ? /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8",
                    children: galleryBuckets.map((bucket) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      type: "button",
                      onClick: () => {
                      ...
                      children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "aspect-[4/5] bg-slate-950 relative overflow-hidden",
                        children: [
                          bucket.fotoPerfil ? /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                            ...
                            onError: applyCryingEmojiFallback
                          }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-xl font-black italic text-white tracking-tighter leading-none",
                                children: bucket.nombre
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-[10px] font-black uppercase tracking-widest text-[var(--metal-gold)] mt-2",
                                children: bucket.profesion || "Perfil"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "mt-3 inline-flex items-center gap-2 px-3 py-1 rounded-full border theme-border-secondary bg-slate-950/85 text-[9px] font-black uppercase tracking-[0.2em] text-slate-200",
                                children: [
                    ...
                    }, bucket.id, false, undefined, this))
                  }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                    children: [
                      galleryViewMode !== "PERSONAJE" && isGalleryBucketMode && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "flex items-center gap-3",
                      ...
                      galleryViewMode === "PERSONAJE" && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "space-y-4",
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                ...
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                    value: "",
                                    disabled: true,
                                    children: availableCharacterBuckets.length ? "Seleccionar personaje…" : "No hay más personajes disponibles"
                                  }, undefined, false, undefined, this),
                                  availableCharacterBuckets.map((bucket) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                    value: bucket.id,
                                    children: [
                                      bucket.nombre,
                                      " · ",
                                  ...
                                  }, bucket.id, true, undefined, this))
                                ]
                              }, undefined, true, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          selectedCharacterBuckets.length > 0 ? /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "flex flex-wrap gap-2",
                            children: selectedCharacterBuckets.map((bucket) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              type: "button",
                            ...
                            className: "text-xs text-slate-400",
                            children: "Seleccioná uno o más personajes para mostrar su multimedia en la galería."
                          }, undefined, false, undefined, this)
                        ]
                      }, undefined, true, undefined, this),
                      galleryViewMode === "ETIQUETA" && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "hud-frame hud-frame--panel flex flex-wrap gap-3 rounded-2xl p-4",
                        children: GALLERY_LABELS.map((label) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                          ...
                          children: [
                            label,
                            " · ",
                            (allGalleryPhotosByLabel[label] || []).length
                          ]
                        }, label, true, undefined, this))
                      }, undefined, false, undefined, this),
                      galleryViewMode === "PERSONAJE" && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            ...
                            children: [
                              ...
                              "Play ",
                              activeGalleryBucket?.nombre || currentGalleryModeLabel
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            type: "button",
                          ...
                          galleryAudioTracks.length > 0 && /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                className: "filter-select min-w-[180px]",
                                value: selectedGalleryAudioA,
                                onChange: (event) => setSelectedGalleryAudioA(event.target.value),
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                    value: "",
                                    children: "Audio principal"
                                  }, undefined, false, undefined, this),
                                  galleryAudioTracks.map((track, index) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                    value: track.url,
                                    children: track.nombre
                                  }, `${track.url}-${index}`, false, undefined, this))
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                className: "filter-select min-w-[180px]",
                                value: selectedGalleryAudioB,
                                onChange: (event) => setSelectedGalleryAudioB(event.target.value),
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                    value: "",
                                    children: "Audio secundario (opcional)"
                                  }, undefined, false, undefined, this),
                                  galleryAudioTracks.map((track, index) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                    value: track.url,
                                    children: track.nombre
                                  }, `${track.url}-b-${index}`, false, undefined, this))
                                ]
                              }, undefined, true, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "inline-flex items-center gap-2 rounded-full border theme-border-secondary bg-slate-950/80 px-3 py-2",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("label", {
                                htmlFor: "galleryPlaybackSeconds",
                                className: "text-[10px] font-black uppercase tracking-[0.15em] text-slate-300",
                                children: "Duración"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                ...
                                children: [3, 5, 7, 10].map((seconds) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                  value: seconds,
                                  children: [
                                    seconds,
                                    " segundos"
                                  ]
                                }, seconds, true, undefined, this))
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this)
                        ]
                      }, undefined, true, undefined, this),
                      filteredGalleryPhotos.length === 0 ? /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "py-24 border border-dashed theme-border-secondary rounded-2xl text-center bg-slate-950/30",
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "w-20 h-20 rounded-full bg-slate-900 border theme-border-secondary flex items-center justify-center mx-auto mb-6",
                            children: /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
                              name: "image-off",
                              size: 28,
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                            className: "font-title text-xl font-black italic text-white tracking-[0.06em]",
                            children: "No hay archivos para mostrar"
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            className: "font-title text-xs font-medium text-slate-500 tracking-[0.06em] mt-3",
                            children: galleryViewMode === "ETIQUETA" && selectedTagLabels.length === 0 ? "Seleccioná una o más etiquetas para ver multimedia." : "Probá con otra etiqueta o quitá el filtro por etiqueta."
                          }, undefined, false, undefined, this)
                        ]
                      }, undefined, true, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4",
                            children: visibleGalleryPhotos.map((photo, index) => {
                              ...
                              return /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  className: "aspect-[3/4] relative overflow-hidden bg-slate-950",
                                  children: [
                                    photo.type === "video" ? (() => {
                                      const embedInfo = getVideoEmbedInfo(photo.url);
                                      if (embedInfo) {
                                        return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                          className: "w-full h-full bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.3),_rgba(15,23,42,0.95))] flex flex-col items-center justify-center gap-4 p-6 text-center",
                                          children: [
                                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                              className: "w-16 h-16 rounded-full border border-[var(--metal-gold)]/40 bg-slate-950/70 flex items-center justify-center text-[color:color-mix(in_srgb,var(--metal-gold)_72%,white)] text-2xl",
                                              children: "▶"
                                            }, undefined, false, undefined, this),
                                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                              children: [
                                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                                  className: "font-title text-sm font-semibold tracking-[0.1em] text-white",
                                                  children: "Video"
                                                }, undefined, false, undefined, this),
                                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                                  className: "font-title text-[10px] font-medium tracking-[0.08em] text-slate-400 mt-2",
                                                  children: embedInfo.provider
                                                }, undefined, false, undefined, this)
                                              ]
                                            }, undefined, true, undefined, this)
                                          ]
                                      ...
                                      return /* @__PURE__ */ jsxDEV_7x81h0kn("video", {
                                        src: photo.url,
                                      ...
                                      src: getSafeImageSrc(photo.url, CRYING_EMOJI_FALLBACK),
                                      alt: `${photo.nombre} - ${photo.label || "galería"}`,
                                      className: "w-full h-full object-cover transition-transform duration-700 group-hover:scale-105",
                                      onError: applyCryingEmojiFallback,
                                      loading: "lazy",
                                      decoding: "async"
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      className: "absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-cyan-950/40 via-transparent to-transparent"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      className: "absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-[#020617] via-[#020617]/60 to-transparent",
                                      children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                        ...
                                        children: [
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                            ...
                                            children: [
                                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                                className: "text-xs sm:text-sm font-black italic text-white tracking-tight leading-none truncate",
                                                title: photo.nombre,
                                                children: photo.nombre
                                              }, undefined, false, undefined, this),
                                              photo.autor ? /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                                className: "text-[10px] font-semibold text-slate-300 mt-1 truncate",
                                                title: photo.autor,
                                                children: [
                                                  "Autor: ",
                                                  photo.autor
                                          ...
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                            className: "px-2 py-[2px] rounded-full border theme-border-secondary bg-slate-950/85 text-[7px] font-black uppercase tracking-[0.18em] text-slate-200 shrink-0",
                                            children: photo.type === "video" ? "VIDEO" : photo.isGif ? "GIF" : "IMG"
                                          }, undefined, false, undefined, this)
                                        ]
                                      }, undefined, true, undefined, this)
                                    }, undefined, false, undefined, this)
                              ...
                              }, photo.id, false, undefined, this);
                            })
                          }, undefined, false, undefined, this),
                          hasMoreGalleryPhotos && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  ...
                  /* @__PURE__ */ jsxDEV_7x81h0kn("audio", {
                    ref: galleryAudioARef,
                    loop: true,
                    preload: "none"
                  }, undefined, false, undefined, this),
                  /* @__PURE__ */ jsxDEV_7x81h0kn("audio", {
                    ref: galleryAudioBRef,
                    loop: true,
                    preload: "none"
                  }, undefined, false, undefined, this),
                  selectedGalleryPhoto && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: [
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: `gallery-viewer-shell w-screen h-screen max-h-screen flex flex-col ${isGeneralFullscreen ? "gap-1 px-1 py-1 sm:px-2 sm:py-2" : "gap-4 px-3 py-3 sm:px-6 sm:py-6"}`,
                        onClick: (event) => event.stopPropagation(),
                        children: [
                          !isGeneralFullscreen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "flex items-center justify-between gap-4 px-1 sm:px-2",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-2xl sm:text-3xl font-black italic text-white tracking-tighter",
                                    children: selectedGalleryPhoto.nombre
                                  }, undefined, false, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-[var(--metal-gold)] mt-2",
                                    children: [
                                      selectedGalleryPhoto.profesion,
                                      selectedGalleryPhoto.nacionalidad ? ` · ${selectedGalleryPhoto.nacionalidad}` : ""
                                    ]
                                  }, undefined, true, undefined, this),
                                  selectedGalleryPhoto.autor ? /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-[11px] sm:text-xs font-semibold text-slate-300 mt-2",
                                    children: [
                                      "Autor: ",
                                      selectedGalleryPhoto.autor
                                    ]
                                  }, undefined, true, undefined, this) : null
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "flex items-center gap-3",
                                children: [
                                  filteredGalleryPhotos.length > 1 && /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                        type: "button",
                                        onClick: () => setIsGalleryPlaying((prev) => !prev),
                                        className: `w-12 h-12 rounded-full border flex items-center justify-center text-2xl transition-all shadow-lg shadow-black/40 ${isGalleryPlaying ? "border-[var(--metal-gold)] bg-[color:color-mix(in_srgb,var(--metal-gold)_35%,rgba(2,6,23,0.9))] brightness-110" : "theme-border-secondary bg-slate-900/85 opacity-80"}`,
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                        ...
                                        className: `w-12 h-12 rounded-full border flex items-center justify-center text-2xl transition-all shadow-lg shadow-black/40 ${isGalleryRandom ? "border-[var(--metal-gold)] bg-[color:color-mix(in_srgb,var(--metal-gold)_35%,rgba(2,6,23,0.9))] brightness-110" : "theme-border-secondary bg-slate-900/85 opacity-80"}`,
                                  ...
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                    className: "px-3 py-2 rounded-full border theme-border-secondary bg-slate-950/85 text-[10px] font-black uppercase tracking-[0.3em] text-slate-200",
                                    children: selectedGalleryPhoto.type === "video" ? "VIDEO" : selectedGalleryPhoto.isGif ? "GIF" : "IMG"
                                  }, undefined, false, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                    ...
                                    children: selectedGalleryPhoto.label || "—"
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this)
                            ]
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: `gallery-viewer-media-wrap relative flex-1 min-h-0 overflow-hidden bg-black/50 ${isGeneralFullscreen ? "rounded-xl md:rounded-2xl" : "md:rounded-[2rem] md:border theme-border-secondary"}`,
                            children: [
                              selectedGalleryPhoto.type === "video" ? (() => {
                                const embedInfo = getVideoEmbedInfo(selectedGalleryPhoto.url);
                                if (embedInfo) {
                                  return /* @__PURE__ */ jsxDEV_7x81h0kn("iframe", {
                              ...
                              filteredGalleryPhotos.length > 1 && /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                                children: isGeneralFullscreen ? /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  className: "absolute left-1/2 bottom-3 sm:bottom-4 -translate-x-1/2 flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 rounded-full border border-[color:color-mix(in_srgb,var(--metal-gold)_40%,rgba(148,163,184,0.45))] bg-slate-950/85 backdrop-blur-md shadow-[0_0_18px_rgba(15,23,42,0.65)]",
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                      type: "button",
                                      onClick: (event) => {
                                        event.stopPropagation();
                                        setIsGalleryRandom((prev) => !prev);
                                      },
                                      className: `px-3 sm:px-4 h-10 rounded-full border text-[11px] sm:text-xs font-black uppercase tracking-[0.18em] transition-all ${isGalleryRandom ? "border-[var(--metal-gold)] text-[var(--metal-gold)] bg-[color:color-mix(in_srgb,var(--metal-gold)_28%,rgba(2,6,23,0.9))]" : "theme-border-secondary text-slate-200 bg-slate-900/90"}`,
                                      "aria-label": isGalleryRandom ? "Aleatorio activado" : "Aleatorio desactivado",
                                      children: isGalleryRandom ? "Random On" : "Random Off"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                      type: "button",
                                      onClick: (event) => {
                                        event.stopPropagation();
                                        setIsGalleryPlaying((prev) => !prev);
                                      },
                                      className: `px-4 sm:px-5 h-10 rounded-full border text-[11px] sm:text-xs font-black uppercase tracking-[0.18em] transition-all ${isGalleryPlaying ? "border-[var(--metal-gold)] text-[var(--metal-gold)] bg-[color:color-mix(in_srgb,var(--metal-gold)_28%,rgba(2,6,23,0.9))]" : "theme-border-secondary text-slate-200 bg-slate-900/90"}`,
                                      "aria-label": isGalleryPlaying ? "Pausar reproducción" : "Reproducir",
                                      children: isGalleryPlaying ? "Pause" : "Play"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                      type: "button",
                                      onClick: (event) => {
                                ...
                                }, undefined, true, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  type: "button",
                                  onClick: (event) => {
                                    event.stopPropagation();
                                    showNextGalleryPhoto();
                          ...
                          !isGeneralFullscreen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-1 sm:px-2",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "flex items-center justify-start sm:justify-end gap-2",
                                children: [
                                  isEditingGalleryLabel ? /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                        type: "url",
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                        ...
                                        children: [
                                          ...
                                          GALLERY_LABELS.map((label) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                            value: label,
                                            children: label
                                          }, label, false, undefined, this))
                                        ]
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                        ...
                                        children: isSavingGalleryEditor ? "Guardando..." : "Guardar"
                                      }, undefined, false, undefined, this),
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                        type: "button",
                                        onClick: () => {
                                    ...
                                    children: [
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        children: selectedGalleryPhoto?.label || "Sin etiqueta"
                                      }, undefined, false, undefined, this)
                                    ]
                                  }, undefined, true, undefined, this),
                                  isEditingGalleryLabel && galleryEditorError && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-[10px] font-bold tracking-[0.04em] text-rose-300",
                                    children: galleryEditorError
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this)
                            ]
                  ...
                  isBrokenGalleryModalOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      className: "w-full max-w-[1200px] max-h-full theme-surface-card border theme-border-secondary rounded-[2rem] p-4 sm:p-6 overflow-hidden",
                      onClick: (event) => event.stopPropagation(),
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  ...
                                  children: "Fotos rotas"
                                }, undefined, false, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: "text-[10px] font-black uppercase tracking-[0.25em] text-[var(--metal-gold)] mt-2",
                                  children: [
                                    brokenGalleryPhotos.length,
                                    " enlace",
                                    brokenGalleryPhotos.length === 1 ? "" : "s",
                                    " sin vista"
                                  ]
                                }, undefined, true, undefined, this)
                              ]
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: !brokenGalleryGroups.length ? /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "theme-surface-soft rounded-2xl border theme-border-secondary p-6 text-center",
                            children: /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-sm font-semibold text-white",
                              children: "No se detectaron fotos rotas ahora mismo."
                            }, undefined, false, undefined, this)
                          }, undefined, false, undefined, this) : brokenGalleryGroups.map((group) => /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                            className: "theme-surface-soft rounded-2xl border theme-border-secondary p-4 sm:p-5",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "mb-4",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-lg font-black italic text-white tracking-tight",
                                    children: group.nombre
                                  }, undefined, false, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-[10px] font-black uppercase tracking-[0.18em] text-slate-400 mt-1",
                                    children: [
                                      group.profesion,
                                      group.nacionalidad ? ` · ${group.nacionalidad}` : ""
                                    ]
                                  }, undefined, true, undefined, this)
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "space-y-3",
                                children: group.fotos.map((photo) => {
                                  ...
                                  return /* @__PURE__ */ jsxDEV_7x81h0kn("article", {
                                    className: "grid grid-cols-1 xl:grid-cols-[76px_minmax(0,1fr)_auto] gap-3 p-3 rounded-xl border theme-border-secondary bg-slate-950/40",
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                        className: "w-[76px] h-[76px] rounded-lg overflow-hidden border theme-border-secondary bg-slate-900 flex items-center justify-center",
                                        children: /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                                          src: CRYING_EMOJI_FALLBACK,
                                          alt: "Vista previa rota",
                                          className: "w-full h-full object-cover"
                                        }, undefined, false, undefined, this)
                                      }, undefined, false, undefined, this),
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                        ...
                                        children: [
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                            ...
                                            children: [
                                              /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                                className: "text-[10px] px-2 py-1 rounded-full border theme-border-secondary bg-slate-900/85 font-black uppercase tracking-[0.16em] text-slate-200",
                                                children: [
                                                  "Etiqueta: ",
                                                  photo.label || "Sin etiqueta"
                                                ]
                                              }, undefined, true, undefined, this),
                                              /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                                className: "text-[10px] px-2 py-1 rounded-full border theme-border-secondary bg-slate-900/85 font-black uppercase tracking-[0.16em] text-slate-400",
                                                children: photo.sourceTag
                                              }, undefined, false, undefined, this)
                                            ]
                                          }, undefined, true, undefined, this),
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                        className: "flex flex-wrap xl:flex-col gap-2 xl:justify-center",
                                        children: [
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                            type: "button",
              ...
              activeTab === "BATALLAS" && !selectedCategory && !selectedArena && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "space-y-10 animate-in fade-in duration-500",
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            ...
                            children: "Resetear calificaciones"
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            onClick: () => setShowResetArenaPicker((prev) => !prev),
                            className: "solid-metal-ui px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-[0.18em] transition-all",
                            children: "Resetear item"
                          }, undefined, false, undefined, this)
                        ]
                      }, undefined, true, undefined, this),
                      showResetArenaPicker && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "theme-surface-card border theme-border-secondary rounded-2xl p-4 flex flex-wrap items-center gap-3",
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                            ...
                            children: ARENAS.map((arenaName) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                              value: arenaName,
                              children: arenaName
                            }, arenaName, false, undefined, this))
                          }, undefined, false, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        children: [
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            className: "text-xs font-bold text-[var(--metal-gold)] uppercase tracking-widest mt-1",
                            children: !selectedBattleScope ? "Paso 1: elegí un modo de enfrentamiento" : requiresBattleGroupSelection && !selectedBattleGroupKey ? `Paso 2: elegí una opción de ${getBattleScopeLabel(selectedBattleScope)}` : `Paso 3: elegí uno de los 15 ítems · Modo: ${getBattleScopeLabel(selectedBattleScope)}`
                          }, undefined, false, undefined, this)
                        ]
                      }, undefined, true, undefined, this)
                    ]
                  }, undefined, true, undefined, this),
                  !selectedBattleScope && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6",
                    children: BATTLE_SCOPES.map((scope) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      onClick: () => {
                  ...
                  selectedBattleScope && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            ...
                            children: [
                              "Modo: ",
                              getBattleScopeLabel(selectedBattleScope)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                            onClick: () => {
                            ...
                            children: "Cambiar modo"
                          }, undefined, false, undefined, this)
                        ]
                      }, undefined, true, undefined, this),
                      requiresBattleGroupSelection && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "theme-surface-card border theme-border-secondary rounded-2xl p-4 space-y-3",
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                            className: "text-[10px] font-black uppercase tracking-[0.2em] text-slate-300",
                            children: [
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "flex flex-wrap gap-2",
                            children: battleScopeOptions.map((option) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              onClick: () => {
                                setSelectedBattleGroupKey(option.key);
                                setSelectedArena(null);
                              },
                              className: `solid-metal-ui px-3 py-2 rounded-xl border text-[10px] font-black uppercase tracking-[0.14em] transition-all ${selectedBattleGroupKey === option.key ? "border-[var(--metal-gold)] text-white" : "theme-border-secondary text-slate-200"}`,
                              children: [
                                option.label,
                                " ",
                                /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                      ...
                      requiresBattleGroupSelection && !selectedBattleGroupKey && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                        className: "text-xs text-slate-400",
                        children: "Seleccioná una opción para habilitar las batallas."
                      }, undefined, false, undefined, this),
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6",
                        children: [
                          ARENAS.map((arenaName) => {
                            ...
                            return /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              ...
                              onClick: () => {
                                ...
                                if (!arenaBattleState[arenaKey])
                                  initArenaBattle(arenaName, selectedBattleScope, selectedBattleGroupKey);
                              },
                              ...
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: `text-[10px] font-black uppercase tracking-[0.28em] ${isArenaCompleted ? "text-emerald-100/90" : "text-slate-500"}`,
                                  children: "Item"
                                }, undefined, false, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                                  className: "font-black italic text-white mt-2",
                                ...
                                isArenaCompleted && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: "battle-mode-card__status mt-4",
                                  children: "Completada"
                                }, undefined, false, undefined, this)
                              ]
                            }, arenaName, true, undefined, this);
                          }),
                          "                    "
                        ]
                      }, undefined, true, undefined, this)
              ...
              activeTab === "BATALLAS" && selectedArena && (() => {
                ...
                return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  ...
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      className: "bg-slate-900/95 backdrop-blur-sm p-4 sm:p-6 rounded-2xl border theme-border-secondary space-y-4",
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "flex flex-wrap items-center gap-3",
                          children: [
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              ...
                              children: "Resetear una batalla"
                            }, undefined, false, undefined, this)
                          ]
                        }, undefined, true, undefined, this),
                        showBattleResetPanel && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "theme-surface-card border border-red-400/30 rounded-2xl p-4 mt-3",
                          children: (() => {
                            const playedKeys = Object.keys(arenaState?.matchups || {}).filter((key) => !!arenaState?.matchups?.[key]);
                            if (!arenaState || !playedKeys.length) {
                              return /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-xs text-red-200 font-bold uppercase tracking-[0.12em]",
                                children: "No hay cruces jugados para resetear."
                              }, undefined, false, undefined, this);
                            }
                            return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "space-y-2",
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                  className: "text-[10px] font-black uppercase tracking-[0.18em] text-red-200",
                                  children: "Elegí el cruce a deshacer:"
                                }, undefined, false, undefined, this),
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
                              className: "font-title text-2xl sm:text-3xl lg:text-4xl font-black italic text-white tracking-[0.08em] break-words leading-tight",
                              children: selectedArena
                            }, undefined, false, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-slate-400 mt-1",
                              children: [
                                "Arena activa · Modo: ",
                                getBattleScopeLabel(selectedBattleScope)
                              ]
                            }, undefined, true, undefined, this),
                            arenaState?.activeGroupLabel && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-[10px] sm:text-xs font-bold uppercase tracking-[0.18em] text-[var(--metal-gold)] mt-2",
                              children: arenaState.activeGroupLabel
                            }, undefined, false, undefined, this)
                          ]
                        }, undefined, true, undefined, this)
                      ]
                    }, undefined, true, undefined, this),
                    !arenaState && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      className: "theme-surface-card border theme-border-secondary rounded-2xl p-8 text-center",
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                          className: "text-sm text-slate-300",
                          children: "Presioná para iniciar esta arena."
                        }, undefined, false, undefined, this),
                        /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                          ...
                          children: "Iniciar batallas"
                        }, undefined, false, undefined, this)
                      ]
                    }, undefined, true, undefined, this),
                    arenaState && arenaState.isFinished && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      className: "theme-surface-card border theme-border-secondary rounded-2xl p-8 text-center",
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                          className: "text-2xl font-black italic text-white",
                          children: "Arena completada"
                        }, undefined, false, undefined, this),
                        /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                          className: "text-slate-400 mt-2",
                          children: "No quedan combates nuevos para este item."
                        }, undefined, false, undefined, this)
                      ]
                    }, undefined, true, undefined, this),
                    arenaState && champion && challenger && !arenaState.isFinished && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      ...
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                          ...
                          children: [
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                              className: "text-2xl sm:text-3xl font-black italic text-white mt-2",
                              children: champion.nombre
                            }, undefined, false, undefined, this),
                            selectedArena === "Estatura" && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-xs text-slate-200 mt-2",
                              children: [
                                "Altura: ",
                                getProfileHeightLabel(champion)
                              ]
                            }, undefined, true, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-xs text-[var(--metal-gold)] mt-3",
                              children: [
                                "Victorias: ",
                                championStats?.wins || 0,
                                " · Batallas: ",
                                championStats?.battles || 0
                              ]
                            }, undefined, true, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "font-title text-4xl text-[var(--metal-gold)] font-black",
                              children: "VS"
                            }, undefined, false, undefined, this),
                            selectedArena === "Estatura" && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-[10px] font-bold uppercase tracking-[0.16em] text-slate-300 mt-3",
                              children: "Compará altura en cm"
                            }, undefined, false, undefined, this)
                          ]
                        }, undefined, true, undefined, this),
                        /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                          ...
                          children: [
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                              className: "text-2xl sm:text-3xl font-black italic text-white mt-2",
                              children: challenger.nombre
                            }, undefined, false, undefined, this),
                            selectedArena === "Estatura" && /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-xs text-slate-200 mt-2",
                              children: [
                                "Altura: ",
                                getProfileHeightLabel(challenger)
                              ]
                            }, undefined, true, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-xs text-[var(--metal-gold)] mt-3",
                              children: [
                                "Victorias: ",
                                challengerStats?.wins || 0,
                                " · Batallas: ",
                                challengerStats?.battles || 0
                              ]
                            }, undefined, true, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                              className: "text-xs text-white/80 mt-1",
              ...
              activeTab === "RANKING" && !selectedCategory && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "theme-surface-card rounded-2xl gothic-frame gothic-frame--ornate animate-in zoom-in-95 duration-500 ranking-table-shell",
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "ranking-table-toolbar",
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      type: "button",
                      onClick: () => setIsFullRankingVisible((prev) => !prev),
                      className: `ranking-toggle-totality ${isFullRankingVisible ? "is-active" : "is-inactive"}`,
                      children: "Ver totalidad"
                    }, undefined, false, undefined, this)
                  }, undefined, false, undefined, this),
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "ranking-table-scroll",
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("table", {
                      className: `w-full ${showExtendedRankingColumns ? "min-w-[860px] xl:min-w-[980px]" : "min-w-[700px] xl:min-w-[820px]"} text-left border-collapse`,
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("thead", {
                          className: "theme-surface-soft",
                          children: /* @__PURE__ */ jsxDEV_7x81h0kn("tr", {
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                className: "px-4 py-6 text-[9px] font-black uppercase tracking-widest text-center rock-carved-text",
                                children: "#"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                className: "px-4 py-6 text-[9px] font-black uppercase tracking-widest rock-carved-text",
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  type: "button",
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  ...
                                  children: [
                                    "Rostro ",
                                    sortBy === "Rostro" ? sortDirection === "asc" ? "↑" : "↓" : ""
                                  ]
                                }, undefined, true, undefined, this)
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  ...
                                  children: [
                                    "Cuerpo ",
                                    sortBy === "Cuerpo" ? sortDirection === "asc" ? "↑" : "↓" : ""
                                  ]
                                }, undefined, true, undefined, this)
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  ...
                                  children: [
                                    "Actitud ",
                                    sortBy === "Actitud" ? sortDirection === "asc" ? "↑" : "↓" : ""
                                  ]
                                }, undefined, true, undefined, this)
                              }, undefined, false, undefined, this),
                              showExtendedRankingColumns && /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                    className: "px-4 py-6 text-[9px] font-black uppercase tracking-widest text-center rock-carved-text",
                                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                      ...
                                      children: [
                                        "Ubicación ",
                                        sortBy === "nacionalidad" ? sortDirection === "asc" ? "↑" : "↓" : ""
                                      ]
                                    }, undefined, true, undefined, this)
                                  }, undefined, false, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("th", {
                                  ...
                                  children: [
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                      ...
                                      children: [
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: "promedio",
                                          children: "G2 SCORE TOTAL (PROMEDIO)"
                                        }, undefined, false, undefined, this),
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("optgroup", {
                                          label: "Categorías Principales",
                                          className: "theme-surface-card text-slate-500",
                                          children: [
                                            /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                              value: "Rostro",
                                              children: "Rostro (Mix)"
                                        ...
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("optgroup", {
                                          ...
                                          className: "theme-surface-card text-slate-500",
                                          children: CARACTERISTICAS.map((cat) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                            value: cat,
                                            children: cat
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("tbody", {
                          children: visibleRankingProfiles.map((p, idx) => /* @__PURE__ */ jsxDEV_7x81h0kn("tr", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                className: "px-8 py-5",
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                  className: `font-black italic rock-carved-text ${idx === 0 ? "text-gold text-4xl" : idx === 1 ? "text-silver text-3xl" : idx === 2 ? "text-bronze text-2xl" : "text-slate-600 text-lg"}`,
                                  children: [
                                    "#",
                                    idx + 1
                                  ]
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  ...
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      className: "w-14 h-14 rounded-xl border-2 p-0.5 transition-all duration-500",
                                      style: {
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      children: [
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                          className: "font-title font-black text-white text-sm tracking-[0.06em] rock-carved-text",
                                          children: p.nombre
                                        }, undefined, false, undefined, this),
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                          className: "font-title text-[10px] font-medium tracking-[0.06em] rock-carved-text",
                                          children: p.profesion
                                        }, undefined, false, undefined, this)
                                      ]
                                    }, undefined, true, undefined, this)
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  ...
                                  children: getRostroScore(p).toFixed(0)
                                }, undefined, false, undefined, this)
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                className: "px-4 py-5 text-center",
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  ...
                                  children: getCuerpoScore(p).toFixed(0)
                                }, undefined, false, undefined, this)
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                className: "px-4 py-5 text-center",
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  ...
                                  children: getActitudScore(p).toFixed(0)
                                }, undefined, false, undefined, this)
                              }, undefined, false, undefined, this),
                              showExtendedRankingColumns && /* @__PURE__ */ jsxDEV_7x81h0kn(Fragment_8vg9x3sq, {
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                    ...
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                        className: "text-[10px] font-bold text-slate-300 uppercase leading-none",
                                        children: p.nacionalidad
                                      }, undefined, false, undefined, this),
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                        className: "text-[8px] font-black text-[var(--metal-gold)]/70 uppercase tracking-tighter",
                                        children: p.ciudad
                                      }, undefined, false, undefined, this)
                                    ]
                                  }, undefined, true, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                    className: "px-4 py-5 text-center font-bold text-[10px] text-slate-400",
                                    children: [
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("td", {
                                ...
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  ...
                                  children: /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                    className: `font-black leading-none ${idx === 0 ? "text-gold" : idx === 1 ? "text-silver" : idx === 2 ? "text-bronze" : "text-[var(--metal-gold)] text-lg"}`,
                                    children: getDisplayedRankingScore(p)
                                  }, undefined, false, undefined, this)
                                }, undefined, false, undefined, this)
                              }, undefined, false, undefined, this)
                            ]
                          }, p.firebaseId || p.id, true, undefined, this))
                        }, undefined, false, undefined, this)
                      ]
                    }, undefined, true, undefined, this)
                  }, undefined, false, undefined, this),
                      ...
                      children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                                    className: "font-title text-xl font-black text-white tracking-wide",
                                    children: [
                                      scoreBreakdownModal.profile.nombre,
                                      " · ",
                                      scoreBreakdownModal.category
                                    ]
                                  }, undefined, true, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-xs text-slate-300 mt-1",
                                    children: "Detalle de enfrentamientos ganados y perdidos."
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                className: "text-[10px] uppercase tracking-[0.18em] text-slate-400 font-black mb-2",
                                children: "Detalle por ítem"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "flex flex-wrap gap-2",
                                children: categoryItems.map((itemName) => (() => {
                                  ...
                                  return /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                    ...
                                    children: [
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                        className: "mt-1 text-[9px] font-semibold text-slate-200/90 tracking-[0.01em] normal-case",
                                        children: [
                                          summary.wins,
                                          "/",
                                          summary.total,
                                  ...
                                  }, `breakdown-item-${itemName}`, true, undefined, this);
                                })())
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          !scoreBreakdownItemDetail?.arenaName && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "grid grid-cols-1 md:grid-cols-2 gap-4",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "rounded-xl border border-emerald-500/40 bg-emerald-950/20 p-4 min-h-44",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("h4", {
                                    className: "text-xs font-black uppercase tracking-[0.16em] text-emerald-300 mb-3",
                                    children: "Ganó contra"
                                  }, undefined, false, undefined, this),
                                  breakdown.wins.length ? /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                    className: "space-y-2",
                                    children: breakdown.wins.map((name, idx) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                                      className: "text-sm text-emerald-200 font-semibold",
                                      children: name
                                    }, `win-${name}-${idx}`, false, undefined, this))
                                  }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-xs text-emerald-200/70",
                                    children: "No hay batallas ganadas registradas."
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "rounded-xl border border-rose-500/40 bg-rose-950/20 p-4 min-h-44",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("h4", {
                                    className: "text-xs font-black uppercase tracking-[0.16em] text-rose-300 mb-3",
                                    children: "Perdió contra"
                                  }, undefined, false, undefined, this),
                                  breakdown.losses.length ? /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                    className: "space-y-2",
                                    children: breakdown.losses.map((name, idx) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                                      className: "text-sm text-rose-200 font-semibold",
                                      children: name
                                    }, `loss-${name}-${idx}`, false, undefined, this))
                                  }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-xs text-rose-200/70",
                                    children: "No hay batallas perdidas registradas."
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this)
                            ]
                          ...
                          scoreBreakdownItemDetail?.arenaName && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "mt-5 rounded-2xl border border-cyan-500/40 bg-cyan-950/15 p-4 max-h-[48vh] overflow-y-auto themed-scrollbar",
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "flex items-center justify-between gap-3 mb-4",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("h4", {
                                    className: "text-xs font-black uppercase tracking-[0.16em] text-cyan-200",
                                    children: [
                                      "Ítem: ",
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "grid grid-cols-1 md:grid-cols-2 gap-4",
                                children: ["wins", "losses"].map((resultType) => {
                                  const isWinList = resultType === "wins";
                                  const battleList = scoreBreakdownItemDetail[resultType] || [];
                                  return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                    className: `rounded-xl border p-3 min-h-36 ${isWinList ? "border-emerald-500/40 bg-emerald-950/15" : "border-rose-500/40 bg-rose-950/15"}`,
                                    children: [
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("h5", {
                                        className: `text-[10px] font-black uppercase tracking-[0.15em] mb-2 ${isWinList ? "text-emerald-300" : "text-rose-300"}`,
                                        children: isWinList ? "Ganadas" : "Perdidas"
                                      }, undefined, false, undefined, this),
                                      battleList.length ? /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                        className: "space-y-2",
                                        children: battleList.map((battle, idx) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                                          className: "flex items-center justify-between gap-2",
                                          children: [
                                            /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                              className: `text-sm font-semibold ${isWinList ? "text-emerald-100" : "text-rose-100"}`,
                                              children: battle.opponentName
                                            }, undefined, false, undefined, this),
                                            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                              type: "button",
                                        ...
                                        }, `${resultType}-${battle.pairKey}-${idx}`, true, undefined, this))
                                      }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                        className: `text-xs ${isWinList ? "text-emerald-200/70" : "text-rose-200/70"}`,
                                        children: isWinList ? "No hay batallas ganadas registradas." : "No hay batallas perdidas registradas."
                                      }, undefined, false, undefined, this)
                                    ]
                                  }, `item-detail-${resultType}`, true, undefined, this);
                                })
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this)
                  ...
                  scorePanelModal.isOpen && scorePanelModal.profile && (() => {
                    ...
                    return /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      ...
                      children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        className: "w-full max-w-3xl theme-surface-card border theme-border-secondary rounded-2xl p-6 space-y-6",
                        onClick: (event) => event.stopPropagation(),
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                children: [
                                  ...
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-xs text-slate-300 mt-1",
                                    children: "Panel de detalle por ítems, características y G2 Score."
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                ...
                                onClick: (event) => {
                                  event.stopPropagation();
                                  setScorePanelModal({ isOpen: false, profile: null });
                                },
                                className: "btn-metal btn-metal--silver px-3 py-2 rounded-lg text-[10px] font-black",
                                children: "Cerrar"
                              }, undefined, false, undefined, this)
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("h4", {
                                className: "text-xs font-black uppercase tracking-[0.16em] text-emerald-300",
                                children: "Ítems"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "flex flex-wrap gap-2",
                                children: CARACTERISTICAS.map((itemKey) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  type: "button",
                                  onClick: (event) => event.stopPropagation(),
                                  className: "px-3 py-2 rounded-lg text-xs font-bold border border-slate-600/80 bg-slate-900/80 text-slate-100",
                                  children: [
                                    itemKey,
                                    ": ",
                                    Number(profileScores[itemKey] || 0).toFixed(0)
                                  ]
                                }, itemKey, true, undefined, this))
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("h4", {
                                className: "text-xs font-black uppercase tracking-[0.16em] text-cyan-300",
                                children: "Características"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "flex flex-wrap gap-2",
                                children: categoryEntries.map(([categoryKey, arenaKeys]) => /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                  type: "button",
                                  onClick: (event) => event.stopPropagation(),
                                  className: "px-3 py-2 rounded-lg text-xs font-bold border border-cyan-500/50 bg-cyan-950/25 text-cyan-100",
                                  children: [
                                    categoryKey,
                                    ": ",
                                    getCategoryScore(selectedProfile, categoryKey).toFixed(0)
                                  ]
                                }, categoryKey, true, undefined, this))
                              }, undefined, false, undefined, this)
                            ]
                          }, undefined, true, undefined, this),
                          /* @__PURE__ */ jsxDEV_7x81h0kn("section", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("h4", {
                                className: "text-xs font-black uppercase tracking-[0.16em] text-[var(--metal-gold)]",
                                children: "G2 Score"
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                type: "button",
                                onClick: (event) => event.stopPropagation(),
                                className: "w-full px-4 py-3 rounded-xl text-sm font-black border border-[var(--metal-gold)]/50 bg-[color:color-mix(in_srgb,var(--metal-gold)_18%,rgba(15,23,42,0.82))] text-[var(--metal-gold)]",
                                children: [
                                  "G2 SCORE TOTAL: ",
                                  calcularPromedio(selectedProfile)
                                ]
                              }, undefined, true, undefined, this)
                            ]
                          }, undefined, true, undefined, this)
              ...
              activeTab === "CATEGORIAS" && !selectedCategory && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                className: "space-y-10 animate-in fade-in duration-500",
                children: [
                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    ...
                    children: [
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        children: /* @__PURE__ */ jsxDEV_7x81h0kn("h2", {
                          className: "text-3xl font-black italic text-white uppercase tracking-tighter",
                          children: "Carpetas"
                        }, undefined, false, undefined, this)
                      }, undefined, false, undefined, this),
                      /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                        ...
                        className: "btn-metal btn-metal--gold px-8 py-3 rounded-xl text-xs",
                        children: "NUEVA CATEGORÍA"
                      }, undefined, false, undefined, this)
                    ]
                  }, undefined, true, undefined, this),
                  categorias.length === 0 ? /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "py-20 border-2 border-dashed theme-border-secondary rounded-2xl text-center",
                    children: /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                      className: "text-slate-600 font-black italic uppercase",
                      children: "No hay categorías personalizadas"
                    }, undefined, false, undefined, this)
                  }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                    className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8",
                    children: categorias.map((cat) => /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      onClick: () => setSelectedCategory(cat.firebaseId),
                      className: "group relative bg-[linear-gradient(180deg,#0b1222_0%,#050a16_100%)] rounded-xl overflow-hidden border theme-border-secondary cursor-pointer hover:border-[var(--metal-gold)]/70 transition-all duration-500 shadow-[inset_0_1px_0_rgba(148,163,184,0.2),0_18px_30px_rgba(2,6,23,0.55)] aspect-[3/4]",
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "absolute top-5 right-5 z-50 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300",
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                              onClick: (e) => {
                                e.stopPropagation();
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                              className: "text-xl font-black text-white uppercase italic tracking-tighter mb-2 drop-shadow-lg",
                              children: cat.label
                            }, undefined, false, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "inline-flex items-center gap-2 bg-[var(--metal-bronze)]/20 backdrop-blur-md px-4 py-1.5 rounded-full border border-[color:color-mix(in_srgb,var(--metal-gold)_30%,transparent)]",
                              children: /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                className: "text-[10px] font-black uppercase text-[var(--metal-gold)] tracking-widest",
                                children: [
                                  perfiles.filter((p) => obtenerCategoriasDePerfil(p).includes(cat.firebaseId)).length,
                                  " Perfiles"
                                ]
                              }, undefined, true, undefined, this)
                            }, undefined, false, undefined, this)
                    ...
                    }, cat.firebaseId, true, undefined, this))
                  }, undefined, false, undefined, this)
                ]
              }, undefined, true, undefined, this)
            ]
      ...
      isModalOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
        className: "fixed inset-0 z-[140] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm",
        children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
          className: "bg-[linear-gradient(180deg,#0b1222_0%,#050a16_100%)] w-full max-w-2xl rounded-3xl overflow-hidden shadow-[inset_0_1px_0_rgba(148,163,184,0.22),0_24px_60px_rgba(2,6,23,0.65)] border theme-border-secondary max-h-[90vh] flex flex-col relative animate-in zoom-in-95 duration-300",
          children: [
            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "w-12 h-12 bg-[var(--metal-bronze)] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-cyan-900/50",
                                children: /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
                                  name: editingId ? "user" : "user-plus"
                                }, undefined, false, undefined, this)
                              }, undefined, false, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                                className: "text-xl font-black italic tracking-tighter text-white",
                                children: editingId ? "Editar Perfil" : "Información del Perfil"
                              }, undefined, false, undefined, this),
                              editingId && /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                type: "button",
                                onClick: () => {
                                  openMultimediaTab({
                                    ...formData,
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            className: "w-48 h-60 rounded-2xl border-2 border-cyan-500 shadow-[0_0_20px_rgba(34,211,238,0.4)] overflow-hidden bg-slate-900 flex items-center justify-center relative group",
                            children: formData.fotos.length > 0 && formData.fotos[0] !== "" ? /* @__PURE__ */ jsxDEV_7x81h0kn("img", {
                              src: getSafeImageSrc(formData.fotos[0], CRYING_EMOJI_FALLBACK),
                              className: "w-full h-full object-cover",
                              alt: "Preview",
                              onError: applyCryingEmojiFallback
                            }, undefined, false, undefined, this) : /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      ...
                      /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                        ...
                        children: [
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "rounded-2xl border border-emerald-400/30 bg-emerald-950/20 p-4",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-[10px] font-black uppercase tracking-[0.2em] text-emerald-300",
                                    children: "Datos actuales"
                                  }, undefined, false, undefined, this),
                                  profileCompletionRows.completed.length > 0 ? /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                    className: "mt-3 space-y-1",
                                    children: profileCompletionRows.completed.map((row) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                                    ...
                                    children: "Todavía no hay datos cargados."
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "rounded-2xl border border-amber-300/30 bg-amber-950/20 p-4",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                                    className: "text-[10px] font-black uppercase tracking-[0.2em] text-amber-200",
                                    children: "Datos faltantes por completar"
                                  }, undefined, false, undefined, this),
                                  profileCompletionRows.missing.length > 0 ? /* @__PURE__ */ jsxDEV_7x81h0kn("ul", {
                                    className: "mt-3 space-y-1",
                                    children: profileCompletionRows.missing.map((row) => /* @__PURE__ */ jsxDEV_7x81h0kn("li", {
                                    ...
                                    className: "mt-3 text-xs text-amber-100/70",
                                    children: "¡Perfil completo en esta sección!"
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this)
                            ]
                          ...
                          /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                            ...
                            children: [
                              /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                ...
                                className: "col-span-2 w-full theme-surface-soft border theme-border-secondary p-5 rounded-xl outline-none focus:ring-2 focus:ring-[var(--glow-gold)] text-white font-bold",
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                ...
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("label", {
                                    className: "text-[9px] font-black text-slate-500 ml-4 uppercase",
                                    children: "URL de la Foto (Avatar)"
                                  }, undefined, false, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                ...
                                children: [
                                  ...
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                    type: "date",
                                    className: "w-full theme-surface-soft border theme-border-secondary p-5 rounded-xl outline-none text-white font-bold",
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                ...
                                children: [
                                  ...
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                    ...
                                    onChange: (e) => setFormData({ ...formData, estaturaCm: e.target.value })
                                  }, undefined, false, undefined, this)
                                ]
                              }, undefined, true, undefined, this),
                              /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                              ...
                              /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                className: "col-span-2 space-y-1 group",
                                children: [
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("label", {
                                    className: "text-[9px] font-black text-[var(--metal-gold)]/60 ml-4 uppercase tracking-widest transition-colors group-focus-within:text-[var(--metal-gold)]",
                                    children: "Profesión"
                                  }, undefined, false, undefined, this),
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                    required: true,
                                    ...
                                    children: [
                                      ...
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                        value: "Bailarina y Atleta",
                                        className: "theme-surface-card",
                                        children: "\uD83D\uDC83\uD83C\uDFCB️‍♀️ Bailarina y Atleta"
                                      }, undefined, false, undefined, this),
                                      /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                        value: "Otro",
                                        className: "theme-surface-card",
                                  ...
                                  /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                    className: "absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--metal-gold)]/50",
                                    children: /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
                                      name: "chevron-down",
                                      size: 18
                                    }, undefined, false, undefined, this)
                ...
                /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  ...
                  children: [
                    editingId && /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      ...
                      className: "btn-metal btn-metal--danger px-10 py-8 rounded-xl text-xs",
                      children: /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
                        name: "trash-2",
                        size: 20
                      }, undefined, false, undefined, this)
                    ...
                    /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                      ...
                      className: "btn-metal btn-metal--gold flex-1 py-8 rounded-xl text-xs disabled:cursor-not-allowed disabled:opacity-60",
                      children: isSavingProfile ? "Guardando..." : editingId ? "Actualizar Registro" : "Guardar Perfil"
                    }, undefined, false, undefined, this)
                  ]
                }, undefined, true, undefined, this)
              ]
      ...
      isDeleteProfileModalOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
        ...
        children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
          className: "w-full max-w-xl theme-surface-card border theme-border-secondary rounded-3xl p-8 space-y-6",
          onClick: (event) => event.stopPropagation(),
          children: [
            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
              className: "space-y-3",
              children: [
                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                  className: "text-xs font-black uppercase tracking-[0.24em] text-red-300",
                  children: "Acción destructiva"
                }, undefined, false, undefined, this),
                /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                ...
                /* @__PURE__ */ jsxDEV_7x81h0kn("p", {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                      ...
                      children: contextProfile?.nombre || "este perfil"
                    }, undefined, false, undefined, this),
                    " y sus datos en la base JSON local. No se puede deshacer."
                  ]
                }, undefined, true, undefined, this),
                profileActionError && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                  className: "rounded-xl border border-red-400/40 bg-red-500/10 px-4 py-3 text-xs font-semibold text-red-200",
                  children: profileActionError
                }, undefined, false, undefined, this)
              ]
            }, undefined, true, undefined, this),
            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
      ...
      isCatModalOpen && /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
        className: "fixed inset-0 z-[140] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm",
        children: /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
          className: "bg-[linear-gradient(180deg,#0b1222_0%,#050a16_100%)] w-full max-w-5xl rounded-3xl overflow-hidden shadow-[inset_0_1px_0_rgba(148,163,184,0.22),0_24px_60px_rgba(2,6,23,0.65)] border theme-border-secondary max-h-[90vh] flex flex-col relative animate-in zoom-in-95 duration-300",
          children: [
            /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
              type: "button",
              onClick: () => {
            ...
            /* @__PURE__ */ jsxDEV_7x81h0kn("form", {
              ...
              children: [
                  ...
                  children: [
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      ...
                      children: [
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "w-12 h-12 bg-[var(--metal-bronze)] rounded-2xl flex items-center justify-center text-white",
                              children: /* @__PURE__ */ jsxDEV_7x81h0kn(LucideIcon, {
                                name: "scroll"
                              }, undefined, false, undefined, this)
                            }, undefined, false, undefined, this),
                            /* @__PURE__ */ jsxDEV_7x81h0kn("h3", {
                              ...
                              children: "Nuevo Archivo del Reino"
                            }, undefined, false, undefined, this)
                          ]
                        }, undefined, true, undefined, this),
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          ...
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                              ...
                              className: "w-full theme-surface-soft border theme-border-secondary p-5 rounded-xl outline-none font-bold text-lg text-white",
                    ...
                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                      ...
                      children: [
                        ...
                        /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                          className: "p-8 bg-slate-950/50 border theme-border-secondary rounded-2xl space-y-6",
                          children: [
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "grid grid-cols-2 gap-4",
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  ...
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("label", {
                                      className: "text-[9px] font-black text-slate-500 ml-4 uppercase",
                                      children: "NACIONALIDADES"
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      className: "flex flex-wrap gap-1 mb-2 min-h-[20px]",
                                      children: catFormData.reglas.nacionalidades.map((n) => /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        className: "bg-[var(--metal-bronze)]/20 text-[var(--metal-gold)] text-[8px] font-black px-2 py-1 rounded-full border border-[color:color-mix(in_srgb,var(--metal-gold)_30%,transparent)] flex items-center gap-1",
                                        children: [
                                          n,
                                          " ",
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                            type: "button",
                                            onClick: () => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, nacionalidades: catFormData.reglas.nacionalidades.filter((x) => x !== n) } }),
                                            children: "×"
                                          }, undefined, false, undefined, this)
                                        ]
                                      }, n, true, undefined, this))
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                      className: "w-full theme-surface-card p-3 rounded-xl text-[10px] font-bold border theme-border-secondary text-white outline-none",
                                      onChange: (e) => {
                                      ...
                                      children: [
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: "",
                                          children: "Añadir país..."
                                        }, undefined, false, undefined, this),
                                        uniqueNacionalidades.filter((n) => n !== "Todas").map((n) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: n,
                                          children: n
                                ...
                                /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  ...
                                  children: [
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      className: "flex flex-wrap gap-1 mb-2 min-h-[20px]",
                                      children: catFormData.reglas.ciudades.map((c) => /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        className: "bg-[var(--metal-bronze)]/20 text-[var(--metal-gold)] text-[8px] font-black px-2 py-1 rounded-full border border-[color:color-mix(in_srgb,var(--metal-gold)_30%,transparent)] flex items-center gap-1",
                                        children: [
                                          c,
                                          " ",
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                            type: "button",
                                            onClick: () => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, ciudades: catFormData.reglas.ciudades.filter((x) => x !== c) } }),
                                            children: "×"
                                          }, undefined, false, undefined, this)
                                        ]
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                      ...
                                      children: [
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: "",
                                          children: "Añadir ciudad..."
                                        }, undefined, false, undefined, this),
                                        uniqueCiudades.filter((c) => c !== "Todas").map((c) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: c,
                                          children: c
                                ...
                                /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  ...
                                  children: [
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      className: "flex flex-wrap gap-1 mb-2 min-h-[20px]",
                                      children: catFormData.reglas.profesiones.map((p) => /* @__PURE__ */ jsxDEV_7x81h0kn("span", {
                                        className: "bg-[var(--metal-bronze)]/20 text-[var(--metal-gold)] text-[8px] font-black px-2 py-1 rounded-full border border-[color:color-mix(in_srgb,var(--metal-gold)_30%,transparent)] flex items-center gap-1",
                                        children: [
                                          p,
                                          " ",
                                          /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                                            type: "button",
                                            onClick: () => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, profesiones: catFormData.reglas.profesiones.filter((x) => x !== p) } }),
                                            children: "×"
                                          }, undefined, false, undefined, this)
                                        ]
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                      ...
                                      children: [
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: "",
                                          children: "Añadir profesión..."
                                        }, undefined, false, undefined, this),
                                        Object.keys(PROFESIONES_CONFIG).map((p) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          value: p,
                                          children: p
                                    ...
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      ...
                                      children: [
                                        ...
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                          type: "number",
                                          placeholder: "18",
                                          className: "w-full theme-surface-card p-4 rounded-2xl outline-none text-xs font-bold border theme-border-secondary text-white",
                                          value: catFormData.reglas.edadMin,
                                          onChange: (e) => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, edadMin: e.target.value } })
                                        }, undefined, false, undefined, this)
                                      ]
                                    }, undefined, true, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                      ...
                                      children: [
                                        ...
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                          ...
                                          placeholder: "30",
                                          className: "w-full theme-surface-card p-4 rounded-2xl outline-none text-xs font-bold border theme-border-secondary text-white",
                                          value: catFormData.reglas.edadMax,
                                          onChange: (e) => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, edadMax: e.target.value } })
                                        }, undefined, false, undefined, this)
                                      ]
                                    }, undefined, true, undefined, this)
                                  ]
                            ...
                            /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                              className: "space-y-1 pt-4 border-t theme-border-secondary",
                              children: [
                                /* @__PURE__ */ jsxDEV_7x81h0kn("label", {
                                  className: "text-[9px] font-black text-slate-500 ml-4 uppercase",
                                  children: "Condición de Score"
                                }, undefined, false, undefined, this),
                                /* @__PURE__ */ jsxDEV_7x81h0kn("div", {
                                  className: "grid grid-cols-2 gap-2 mb-2",
                                  children: [
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                      className: "theme-surface-card p-3 rounded-xl text-[10px] font-bold outline-none border theme-border-secondary text-white",
                                      value: catFormData.reglas.caracteristica,
                                      onChange: (e) => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, caracteristica: e.target.value } }),
                                      children: CARACTERISTICAS.map((c) => /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                        children: c
                                      }, c, false, undefined, this))
                                    }, undefined, false, undefined, this),
                                    /* @__PURE__ */ jsxDEV_7x81h0kn("select", {
                                      className: "theme-surface-card p-3 rounded-xl text-[10px] font-bold outline-none border theme-border-secondary text-white",
                                      value: catFormData.reglas.operador,
                                      onChange: (e) => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, operador: e.target.value } }),
                                      children: [
                                        /* @__PURE__ */ jsxDEV_7x81h0kn("option", {
                                          children: "Superior a"
                                        }, undefined, false, undefined, this),
                                ...
                                /* @__PURE__ */ jsxDEV_7x81h0kn("input", {
                                  type: "number",
                                  placeholder: "Umbral (0-10)",
                                  className: "w-full theme-surface-card p-4 rounded-2xl outline-none text-xs font-bold border theme-border-secondary text-[var(--metal-gold)]",
                                  value: catFormData.reglas.umbral,
                                  onChange: (e) => setCatFormData({ ...catFormData, reglas: { ...catFormData.reglas, umbral: e.target.value } })
                                }, undefined, false, undefined, this)
                              ]
                            }, undefined, true, undefined, this)
                          ]
                ...
                /* @__PURE__ */ jsxDEV_7x81h0kn("button", {
                  type: "submit",
                  className: "btn-metal btn-metal--gold w-full py-8 rounded-xl text-xs",
                  children: "CREAR CARPETA INTELIGENTE"
                }, undefined, false, undefined, this)
              ]
            }, undefined, true, undefined, this)
          ]
...
root.render(/* @__PURE__ */ jsxDEV_7x81h0kn(App, {}, undefined, false, undefined, this));
