import{jsxDEV as Q,Fragment as wJ}from"react/jsx-dev-runtime";const Z0={apiKey:"AIzaSyAcW679YdIT03pyq8IgKKnR8oyO4_fvpFs",authDomain:"g2elite-a3b85.firebaseapp.com",databaseURL:"https://g2elite-a3b85-default-rtdb.firebaseio.com/",projectId:"g2elite-a3b85",storageBucket:"g2elite-a3b85.firebasestorage.app",messagingSenderId:"951346482286",appId:"1:951346482286:web:6001135252de2a4d982a7f"};if(!firebase.apps.length)firebase.initializeApp(Z0);const K=firebase.database(),{useState:T,useEffect:i,useMemo:u,useRef:mJ}=React,JJ=["C","P","B","N","S","E","X"],oQ="anonimo/galeria",DJ="__anonimo_gallery__",$0=["PERSONAJE","ETIQUETA","GENERAL"],uY={PERSONAJE:"Personaje",ETIQUETA:"Etiqueta",GENERAL:"General"},uJ=[{id:"perfil",label:"Perfil",labels:[]},{id:"colaPiernas",label:"Cola/Piernas",labels:["C"]},{id:"pechos",label:"Pechos",labels:["P"]},{id:"cuerpoCintura",label:"Cintura/Cuerpo",labels:["N","B"]},{id:"sensualidad",label:"Sensualidad",labels:["E","S"]}],H0={perfil:"perfil",pecho:"pechos",pechos:"pechos",cola:"colaPiernas",piernas:"colaPiernas",cuerpo:"cuerpoCintura",cintura:"cuerpoCintura",sensualidad:"sensualidad"},GX=()=>uJ.reduce((_,W)=>{_[W.id]="";return _},{}),qJ=(_={})=>{const W=GX(),B=_&&typeof _==="object"?_:{};return Object.keys(W).reduce((D,L)=>{D[L]=typeof B[L]==="string"?B[L].trim():"";return D},{...W})},tQ=(_="")=>uJ.find((W)=>W.id===_),j0=(_="")=>H0[(_||"").trim().toLowerCase()]||"",aQ={C:{bg:"linear-gradient(135deg, rgba(34,211,238,0.92), rgba(14,116,144,0.95))",border:"rgba(103,232,249,0.95)",glow:"rgba(34,211,238,0.8)",text:"#ecfeff"},P:{bg:"linear-gradient(135deg, rgba(244,114,182,0.92), rgba(157,23,77,0.95))",border:"rgba(251,207,232,0.95)",glow:"rgba(244,114,182,0.78)",text:"#fff1f2"},B:{bg:"linear-gradient(135deg, rgba(251,191,36,0.94), rgba(180,83,9,0.95))",border:"rgba(253,224,71,0.98)",glow:"rgba(251,191,36,0.82)",text:"#fffbea"},N:{bg:"linear-gradient(135deg, rgba(163,230,53,0.92), rgba(63,98,18,0.95))",border:"rgba(217,249,157,0.95)",glow:"rgba(163,230,53,0.78)",text:"#f7fee7"},S:{bg:"linear-gradient(135deg, rgba(192,132,252,0.92), rgba(107,33,168,0.95))",border:"rgba(233,213,255,0.95)",glow:"rgba(192,132,252,0.8)",text:"#faf5ff"},E:{bg:"linear-gradient(135deg, rgba(248,113,113,0.92), rgba(153,27,27,0.95))",border:"rgba(254,202,202,0.95)",glow:"rgba(248,113,113,0.8)",text:"#fef2f2"},X:{bg:"linear-gradient(135deg, rgba(239,68,68,0.98), rgba(127,29,29,0.98))",border:"rgba(254,226,226,1)",glow:"rgba(239,68,68,0.95)",text:"#fff7f7"},DEFAULT:{bg:"linear-gradient(135deg, rgba(51,65,85,0.92), rgba(15,23,42,0.95))",border:"rgba(148,163,184,0.85)",glow:"rgba(148,163,184,0.45)",text:"#f8fafc"}},aJ=(_,W)=>{if(!W)return 0;return(_+W)%W},fY=(_,W,B=!1)=>{const D=Array.isArray(W)?W.length:0;if(!D)return null;if(D===1)return 0;if(B){const L=W.map((v,f)=>({item:v,idx:f})).filter(({item:v,idx:f})=>f!==_&&v&&v.url).map(({idx:v})=>v);if(!L.length)return aJ((_??0)+1,D);return L[Math.floor(Math.random()*L.length)]}return aJ((_??0)+1,D)},fJ=(_)=>aQ[_]||aQ.DEFAULT,rQ=(_,W=!1)=>{const B=_==="INICIAL"?aQ.DEFAULT:_==="100"?aQ.B:fJ(_);return{"--btn-neon-color":B.glow,borderColor:B.border,color:B.text,boxShadow:W?`inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${B.border}, 0 0 20px ${B.glow}`:`inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${B.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${B.glow} 60%, transparent)`,textShadow:`0 0 8px color-mix(in srgb, ${B.glow} 70%, transparent)`,filter:W?"brightness(1.14)":"brightness(1)"}},cY=/\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i,q0=/\.gif(\?.*)?$/i,_0=/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i,O0=/vimeo\.com\/(?:video\/)?(\d+)/i,rJ=(_)=>{const W=(_||"").trim();if(!W)return null;const B=W.match(_0);if(B)return{provider:"youtube",src:`https://www.youtube.com/embed/${B[1]}`,title:"Video de YouTube"};const D=W.match(O0);if(D)return{provider:"vimeo",src:`https://player.vimeo.com/video/${D[1]}`,title:"Video de Vimeo"};return null},W0=["eporner.com"],sY=(_="")=>{const W=String(_||"").trim();if(!W||W.startsWith("data:")||W.startsWith("blob:"))return!1;try{const D=new URL(W).hostname.toLowerCase();return W0.some((L)=>D===L||D.endsWith(`.${L}`))}catch{return!1}},HJ=(_="",W="")=>{const B=String(_||"").trim();if(!B||sY(B))return W;return B},cJ=(_="",W="")=>{if(W==="video"||W==="image")return W;const B=(_||"").trim();if(cY.test(B)||rJ(B))return"video";return"image"},SX=(_="")=>{return typeof _==="string"?_.trim():""},d=(_,W="")=>{if(typeof _==="string")return{url:HJ(_.trim(),""),label:"",type:cJ(_,W),autor:""};if(_&&typeof _==="object"){const B=HJ((_.url||"").trim(),"");return{url:B,label:JJ.includes(_.label)?_.label:"",type:cJ(B,_.type||W),autor:SX(_.autor)}}return{url:"",label:"",type:cJ("",W),autor:""}},E0=(_)=>d(_).url,dY=(_)=>d(_).label,sJ=(_)=>d(_).type,xX=(_,W,B="image")=>Array.isArray(_?.[W])?_[W].map((D)=>d(D,B)).filter((D)=>D.url):[],L0=(_={})=>({firebaseId:DJ,nombre:"Anónimo",nacionalidad:"",ciudad:"",profesion:"Galería",fechaNacimiento:"",estaturaCm:"",fotos:[],galeria:{fotos:xX(_,"fotos","image"),gifs:xX(_,"gifs","image"),videos:xX(_,"videos","video")},batallaFotosPreferidas:GX(),puntuaciones:VQ(),isAnonymousGallery:!0}),F0=(_={})=>{if(_?.isAnonymousGallery||_?.firebaseId===DJ)return"anonimo";return _?.firebaseId||""},U0=(_={})=>{const W=HJ(String(_?.fotos?.[0]||"").trim(),""),B=qJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!W)return!1;return uJ.filter((D)=>D.id!=="perfil").every((D)=>Boolean(String(B?.[D.id]||"").trim()))},B0=(_={})=>{const W=[],B=HJ(String(_?.fotos?.[0]||"").trim(),""),D=qJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!B)W.push("Perfil");uJ.filter((L)=>L.id!=="perfil").forEach((L)=>{if(!String(D?.[L.id]||"").trim())W.push(L.label)});return W},T0=(_=null)=>{if(!_)return;const W=window.open("","_blank");if(!W)return;const B=_?.firebaseId||_?.id||"",D=Array.isArray(_?.galeria?.fotos)?_.galeria.fotos.map((C)=>d(C,"image")).filter((C)=>C.url).map((C,N)=>({...C,sourceTag:"fotos",sourceIndex:N})):[],L=D.filter((C)=>C.type==="image"&&C.sourceTag==="fotos"),v=qJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{}),f=HJ(String(_?.fotos?.[0]||_?.foto||"").trim(),""),QJ=D.filter((C)=>C.type==="image"&&C.url).reduce((C,N)=>{C[N.url]=N;return C},{}),c=D.length?D.map((C,N)=>`
                ...
                `).join(""):'<p class="text-slate-300">Sin imágenes en galería.</p>',U=uJ.map((C)=>{const N=C.id==="perfil",G=N?f:v[C.id]||"",z=G?N?{url:G}:QJ[G]||{url:G,label:"",sourceIndex:-1}:null,_J=Boolean(G);return`
                ...
                <html lang="es">
                    ...
                    <body class="text-slate-200">
                        ...
                        <script>
                            ...
                            const slotConfigById = ${JSON.stringify(uJ.reduce((C,N)=>{C[N.id]=N;return C},{}))};
            ...
            <body>
                ...
                <details open style="width:100%; margin-bottom: 20px; border-radius: 12px; border: 1px solid rgba(148,163,184,0.28); background: rgba(2,6,23,0.45);">
                    ...
                    ${uJ.map((U)=>{const C=U.id==="perfil",N=C?c:QJ[U.id]||"",G=Boolean(N),z=!C;return`
                ...
                <div id="fullscreenViewer" class="fullscreen-viewer">
                    ...
                    <div class="viewer-stage" id="viewerStage">
                        ${P.map((U,C)=>{const N=sJ(U),G=rJ(U.url);return`
