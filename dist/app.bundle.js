import{jsxDEV as Q,Fragment as GJ}from"react/jsx-dev-runtime";const H0={apiKey:"AIzaSyAcW679YdIT03pyq8IgKKnR8oyO4_fvpFs",authDomain:"g2elite-a3b85.firebaseapp.com",databaseURL:"https://g2elite-a3b85-default-rtdb.firebaseio.com/",projectId:"g2elite-a3b85",storageBucket:"g2elite-a3b85.firebasestorage.app",messagingSenderId:"951346482286",appId:"1:951346482286:web:6001135252de2a4d982a7f"};if(!firebase.apps.length)firebase.initializeApp(H0);const K=firebase.database(),{useState:T,useEffect:i,useMemo:u,useRef:mJ}=React,JJ=["C","P","B","N","S","E","X"],oQ="anonimo/galeria",DJ="__anonimo_gallery__",j0=["PERSONAJE","ETIQUETA","GENERAL"],uY={PERSONAJE:"Personaje",ETIQUETA:"Etiqueta",GENERAL:"General"},uJ=[{id:"perfil",label:"Perfil",labels:[]},{id:"colaPiernas",label:"Cola/Piernas",labels:["C"]},{id:"pechos",label:"Pechos",labels:["P"]},{id:"cuerpoCintura",label:"Cintura/Cuerpo",labels:["N","B"]},{id:"sensualidad",label:"Sensualidad",labels:["E","S"]}],q0={perfil:"perfil",pecho:"pechos",pechos:"pechos",cola:"colaPiernas",piernas:"colaPiernas",cuerpo:"cuerpoCintura",cintura:"cuerpoCintura",sensualidad:"sensualidad"},wX=()=>uJ.reduce((_,W)=>{_[W.id]="";return _},{}),qJ=(_={})=>{const W=wX(),B=_&&typeof _==="object"?_:{};return Object.keys(W).reduce((D,L)=>{D[L]=typeof B[L]==="string"?B[L].trim():"";return D},{...W})},tQ=(_="")=>uJ.find((W)=>W.id===_),_0=(_="")=>q0[(_||"").trim().toLowerCase()]||"",aQ={C:{bg:"linear-gradient(135deg, rgba(34,211,238,0.92), rgba(14,116,144,0.95))",border:"rgba(103,232,249,0.95)",glow:"rgba(34,211,238,0.8)",text:"#ecfeff"},P:{bg:"linear-gradient(135deg, rgba(244,114,182,0.92), rgba(157,23,77,0.95))",border:"rgba(251,207,232,0.95)",glow:"rgba(244,114,182,0.78)",text:"#fff1f2"},B:{bg:"linear-gradient(135deg, rgba(251,191,36,0.94), rgba(180,83,9,0.95))",border:"rgba(253,224,71,0.98)",glow:"rgba(251,191,36,0.82)",text:"#fffbea"},N:{bg:"linear-gradient(135deg, rgba(163,230,53,0.92), rgba(63,98,18,0.95))",border:"rgba(217,249,157,0.95)",glow:"rgba(163,230,53,0.78)",text:"#f7fee7"},S:{bg:"linear-gradient(135deg, rgba(192,132,252,0.92), rgba(107,33,168,0.95))",border:"rgba(233,213,255,0.95)",glow:"rgba(192,132,252,0.8)",text:"#faf5ff"},E:{bg:"linear-gradient(135deg, rgba(248,113,113,0.92), rgba(153,27,27,0.95))",border:"rgba(254,202,202,0.95)",glow:"rgba(248,113,113,0.8)",text:"#fef2f2"},X:{bg:"linear-gradient(135deg, rgba(239,68,68,0.98), rgba(127,29,29,0.98))",border:"rgba(254,226,226,1)",glow:"rgba(239,68,68,0.95)",text:"#fff7f7"},DEFAULT:{bg:"linear-gradient(135deg, rgba(51,65,85,0.92), rgba(15,23,42,0.95))",border:"rgba(148,163,184,0.85)",glow:"rgba(148,163,184,0.45)",text:"#f8fafc"}},aJ=(_,W)=>{if(!W)return 0;return(_+W)%W},fY=(_,W,B=!1)=>{const D=Array.isArray(W)?W.length:0;if(!D)return null;if(D===1)return 0;if(B){const L=W.map((b,f)=>({item:b,idx:f})).filter(({item:b,idx:f})=>f!==_&&b&&b.url).map(({idx:b})=>b);if(!L.length)return aJ((_??0)+1,D);return L[Math.floor(Math.random()*L.length)]}return aJ((_??0)+1,D)},fJ=(_)=>aQ[_]||aQ.DEFAULT,rQ=(_,W=!1)=>{const B=_==="INICIAL"?aQ.DEFAULT:_==="100"?aQ.B:fJ(_);return{"--btn-neon-color":B.glow,borderColor:B.border,color:B.text,boxShadow:W?`inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${B.border}, 0 0 20px ${B.glow}`:`inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${B.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${B.glow} 60%, transparent)`,textShadow:`0 0 8px color-mix(in srgb, ${B.glow} 70%, transparent)`,filter:W?"brightness(1.14)":"brightness(1)"}},cY=/\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i,O0=/\.gif(\?.*)?$/i,W0=/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i,E0=/vimeo\.com\/(?:video\/)?(\d+)/i,rJ=(_)=>{const W=(_||"").trim();if(!W)return null;const B=W.match(W0);if(B)return{provider:"youtube",src:`https://www.youtube.com/embed/${B[1]}`,title:"Video de YouTube"};const D=W.match(E0);if(D)return{provider:"vimeo",src:`https://player.vimeo.com/video/${D[1]}`,title:"Video de Vimeo"};return null},L0=["eporner.com"],sY=(_="")=>{const W=String(_||"").trim();if(!W||W.startsWith("data:")||W.startsWith("blob:"))return!1;try{const D=new URL(W).hostname.toLowerCase();return L0.some((L)=>D===L||D.endsWith(`.${L}`))}catch{return!1}},HJ=(_="",W="")=>{const B=String(_||"").trim();if(!B||sY(B))return W;return B},cJ=(_="",W="")=>{if(W==="video"||W==="image")return W;const B=(_||"").trim();if(cY.test(B)||rJ(B))return"video";return"image"},SX=(_="")=>{return typeof _==="string"?_.trim():""},d=(_,W="")=>{if(typeof _==="string")return{url:HJ(_.trim(),""),label:"",type:cJ(_,W),autor:""};if(_&&typeof _==="object"){const B=HJ((_.url||"").trim(),"");return{url:B,label:JJ.includes(_.label)?_.label:"",type:cJ(B,_.type||W),autor:SX(_.autor)}}return{url:"",label:"",type:cJ("",W),autor:""}},F0=(_)=>d(_).url,dY=(_)=>d(_).label,sJ=(_)=>d(_).type,xX=(_,W,B="image")=>Array.isArray(_?.[W])?_[W].map((D)=>d(D,B)).filter((D)=>D.url):[],U0=(_={})=>({firebaseId:DJ,nombre:"Anónimo",nacionalidad:"",ciudad:"",profesion:"Galería",fechaNacimiento:"",estaturaCm:"",fotos:[],galeria:{fotos:xX(_,"fotos","image"),gifs:xX(_,"gifs","image"),videos:xX(_,"videos","video")},batallaFotosPreferidas:wX(),puntuaciones:CQ(),isAnonymousGallery:!0}),B0=(_={})=>{if(_?.isAnonymousGallery||_?.firebaseId===DJ)return"anonimo";return _?.firebaseId||""},T0=(_={})=>{const W=HJ(String(_?.fotos?.[0]||"").trim(),""),B=qJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!W)return!1;return uJ.filter((D)=>D.id!=="perfil").every((D)=>Boolean(String(B?.[D.id]||"").trim()))},C0=(_={})=>{const W=[],B=HJ(String(_?.fotos?.[0]||"").trim(),""),D=qJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{});if(!B)W.push("Perfil");uJ.filter((L)=>L.id!=="perfil").forEach((L)=>{if(!String(D?.[L.id]||"").trim())W.push(L.label)});return W},V0=(_=null)=>{if(!_)return;const W=window.open("","_blank");if(!W)return;const B=_?.firebaseId||_?.id||"",D=Array.isArray(_?.galeria?.fotos)?_.galeria.fotos.map((V)=>d(V,"image")).filter((V)=>V.url).map((V,N)=>({...V,sourceTag:"fotos",sourceIndex:N})):[],L=D.filter((V)=>V.type==="image"&&V.sourceTag==="fotos"),b=qJ(_?.batallaFotosPreferidas||_?.galeria?.battlePhotoPreferences||{}),f=HJ(String(_?.fotos?.[0]||_?.foto||"").trim(),""),QJ=D.filter((V)=>V.type==="image"&&V.url).reduce((V,N)=>{V[N.url]=N;return V},{}),c=D.length?D.map((V,N)=>`
                    <button
                        type="button"
                    ...
                    >
                        <div class="multimedia-thumb-wrap">
                            <img src="${V.url}" alt="Multimedia ${N+1}" style="width:100%;height:100%;object-fit:contain;" loading="lazy" />
                            <span class="multimedia-thumb-label">${V.label||"SIN ETIQUETA"}</span>
                        </div>
                    </button>
                `).join(""):'<p class="text-slate-300">Sin contenido en galería.</p>',P=L.length?L.map((V,N)=>`
                    <button
                        type="button"
                    ...
                    >
                        <div class="multimedia-thumb-wrap multimedia-thumb-wrap--broken">
                            <img src="${V.url}" alt="Imagen rota ${N+1}" style="width:100%;height:100%;object-fit:contain;" loading="lazy" />
                            <span class="multimedia-thumb-label">${V.label||"SIN ETIQUETA"}</span>
                        </div>
                    </button>
                `).join(""):'<p class="text-slate-300">Sin imágenes en galería.</p>',U=uJ.map((V)=>{const N=V.id==="perfil",w=N?f:b[V.id]||"",z=w?N?{url:w}:QJ[w]||{url:w,label:"",sourceIndex:-1}:null,_J=Boolean(w);return`
                    <div class="multimedia-slot-card ${_J?"is-assigned":"is-missing"}" data-slot-id="${V.id}">
                        ...
                        <div class="multimedia-slot-preview">
                            ${_J?`<img src="${z.url}" alt="${V.label}" loading="lazy" />`:'<span class="multimedia-slot-empty">Sin foto designada</span>'}
                        </div>
                        <div class="multimedia-slot-actions">
                            ${!N?`<button type="button" class="multimedia-slot-assign-btn" data-slot-assign="${V.id}">DESIGNAR FOTO</button>`:""}
                ...
                <html lang="es">
                    ...
                    <body class="text-slate-200">
                        ...
                        <script>
                            ...
                            const slotConfigById = ${JSON.stringify(uJ.reduce((V,N)=>{V[N.id]=N;return V},{}))};
            ...
            <body>
                ...
                <details open style="width:100%; margin-bottom: 20px; border-radius: 12px; border: 1px solid rgba(148,163,184,0.28); background: rgba(2,6,23,0.45);">
                    ...
                    ${uJ.map((U)=>{const V=U.id==="perfil",N=V?c:QJ[U.id]||"",w=Boolean(N),z=!V;return`
                            <div class="gallery-slot-card" data-slot-id="${U.id}" style="border:1px solid ${w?"rgba(74,222,128,0.95)":"rgba(248,113,113,0.95)"}; border-radius:10px; padding:10px; background:${w?"linear-gradient(155deg, rgba(20,83,45,0.95) 0%, rgba(21,128,61,0.82) 45%, rgba(5,46,22,0.96) 100%)":"linear-gradient(155deg, rgba(127,29,29,0.96) 0%, rgba(185,28,28,0.84) 45%, rgba(69,10,10,0.96) 100%)"}; box-shadow: inset 0 2px 0 rgba(255,255,255,0.24), inset 0 -2px 0 rgba(2,6,23,0.45), inset 0 0 0 1px ${w?"rgba(187,247,208,0.25)":"rgba(254,202,202,0.22)"}, 0 8px 16px rgba(2,6,23,0.45), 0 0 18px ${w?"rgba(74,222,128,0.35)":"rgba(248,113,113,0.3)"}; position: relative;">
                                ...
                                <div style="margin-top:8px; border-radius:8px; overflow:hidden; aspect-ratio:4/3; border:1px dashed rgba(148,163,184,0.45); background: rgba(2,6,23,0.86); display:flex; align-items:center; justify-content:center;">
                                    ${w?`<img src="${N}" alt="Vista previa ${U.label}" onerror="${PX}" style="width:100%; height:100%; object-fit:cover; display:block;" />`:'<span style="font-size:10px; color:#fca5a5; text-transform:uppercase; letter-spacing:0.08em; font-weight:700; text-align:center; padding:0 8px;">Sin foto designada</span>'}
                                </div>
                    ...
                    ${P.length?P.map((U,V)=>{const N=F0(U),w=dY(U),z=fJ(w),_J={CANTANTE:{color:"#0ea5e9",sombra:"rgba(14,165,233,0.8)"},NSFW:{color:"#ef4444",sombra:"rgba(239,68,68,0.8)"},ACTRIZ:{color:"#22c55e",sombra:"rgba(34,197,94,0.8)"},MODELO:{color:"#f97316",sombra:"rgba(249,115,22,0.8)"},BAILARINA:{color:"#a855f7",sombra:"rgba(168,85,247,0.8)"},INFLUENCER:{color:"#ec4899",sombra:"rgba(236,72,153,0.8)"},PERIODISTA:{color:"#facc15",sombra:"rgba(250,204,21,0.8)"},ATLETA:{color:"#78350f",sombra:"rgba(120,53,15,0.8)"},OTRO:{color:"#ffffff",sombra:"rgba(255,255,255,0.8)"},DEFAULT:{color:"#334155",sombra:"transparent"}},g=_J[B?.toUpperCase()]||_J.DEFAULT;return`
                        ...
                        >
                           <button
                            ...
                            type="button"
                        >✕</button>
                        ${sJ(U)==="video"?(()=>{const VJ=rJ(N);if(VJ)return`<div style="width: 100%; height: 100%; background: radial-gradient(circle at top, rgba(34,211,238,0.35), rgba(15,23,42,0.96)); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; padding: 18px; box-sizing:border-box; text-align:center;"><div style="width:64px; height:64px; border-radius:999px; border:1px solid rgba(34,211,238,0.5); background:rgba(15,23,42,0.75); display:flex; align-items:center; justify-content:center; color:#22d3ee; font-size:28px;">▶</div><div style="font-size:12px; font-weight:800; letter-spacing:0.12em; text-transform:uppercase; color:#e2e8f0;">Video embebido</div><div style="font-size:11px; color:#94a3b8; max-width:100%; word-break:break-word;">${VJ.provider.toUpperCase()}</div></div>`;return`<video src="${N}" style="width: 100%; height: 100%; object-fit: cover;" muted playsinline preload="metadata"></video>`})():`<img src="${N}" alt="Imagen de la galería" onerror="${PX}" style="width: 100%; height: 100%; object-fit: cover;" />`}
                        ...
                        ${w?`<div style="position:absolute; left: 50%; bottom: 10px; transform: translateX(-50%); z-index: 15; min-width: 52px; height: 30px; padding: 0 14px; border-radius: 999px; background: ${z.bg}; border: 1px solid ${z.border}; color: ${z.text}; display:flex; align-items:center; justify-content:center; font-size: 12px; font-weight: 900; letter-spacing: 0.24em; box-shadow: 0 0 14px ${z.glow}, 0 0 24px ${z.glow}; text-shadow: 0 0 10px ${z.glow}; backdrop-filter: blur(8px);">${w}</div>`:""}
                ...
                <div id="fullscreenViewer" class="fullscreen-viewer">
                    ...
                    <div class="viewer-stage" id="viewerStage">
                        ${P.map((U,V)=>{const N=sJ(U),w=rJ(U.url);return`
                            <div class="viewer-slide" id="viewer-slide-${V}">
                                ${N==="video"?w?`<iframe src="${w.src}" title="${w.title} ${V+1}" style="width:min(92vw, 1400px); height:min(75vh, 820px); border:0; border-radius:24px; box-shadow:0 0 40px rgba(34, 211, 238, 0.2); background:#000;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`:`<video src="${U.url}" controls playsinline preload="metadata" style="max-width:min(92vw, 1400px); max-height:calc(100vh - 64px); width:auto; height:auto; border-radius:24px; box-shadow:0 0 40px rgba(34, 211, 238, 0.2); background:#000;"></video>`:`<img src="${U.url}" alt="Vista completa ${V+1}" onerror="${PX}" />`}
                            </div>`}).join("")}
                    </div>
                </div>
