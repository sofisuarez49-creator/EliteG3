import{jsxDEV as Q,Fragment as KJ}from"react/jsx-dev-runtime";const N0={apiKey:"AIzaSyAcW679YdIT03pyq8IgKKnR8oyO4_fvpFs",authDomain:"g2elite-a3b85.firebaseapp.com",databaseURL:"https://g2elite-a3b85-default-rtdb.firebaseio.com/",projectId:"g2elite-a3b85",storageBucket:"g2elite-a3b85.firebasestorage.app",messagingSenderId:"951346482286",appId:"1:951346482286:web:6001135252de2a4d982a7f"};if(!firebase.apps.length)firebase.initializeApp(N0);const M=firebase.database(),{useState:B,useEffect:d,useMemo:u,useRef:RJ}=React,JJ=["C","P","B","N","S","E","X"],NQ="anonimo/galeria",NJ="__anonimo_gallery__",M0=["PERSONAJE","ETIQUETA","GENERAL"],QZ={PERSONAJE:"Personaje",ETIQUETA:"Etiqueta",GENERAL:"General"},sJ=[{id:"perfil",label:"Perfil",labels:[]},{id:"colaPiernas",label:"Cola/Piernas",labels:["C"]},{id:"pechos",label:"Pechos",labels:["P"]},{id:"cuerpoCintura",label:"Cintura/Cuerpo",labels:["N","B"]},{id:"sensualidad",label:"Sensualidad",labels:["E","S"]}],K0={perfil:"perfil",pecho:"pechos",pechos:"pechos",cola:"colaPiernas",piernas:"colaPiernas",cuerpo:"cuerpoCintura",cintura:"cuerpoCintura",sensualidad:"sensualidad"},mX=()=>sJ.reduce((j,W)=>{j[W.id]="";return j},{}),qJ=(j={})=>{const W=mX(),V=j&&typeof j==="object"?j:{};return Object.keys(W).reduce((T,L)=>{T[L]=typeof V[L]==="string"?V[L].trim():"";return T},{...W})},YX=(j="")=>sJ.find((W)=>W.id===j),w0=(j="")=>K0[(j||"").trim().toLowerCase()]||"",ZX={C:{bg:"linear-gradient(135deg, rgba(34,211,238,0.92), rgba(14,116,144,0.95))",border:"rgba(103,232,249,0.95)",glow:"rgba(34,211,238,0.8)",text:"#ecfeff"},P:{bg:"linear-gradient(135deg, rgba(244,114,182,0.92), rgba(157,23,77,0.95))",border:"rgba(251,207,232,0.95)",glow:"rgba(244,114,182,0.78)",text:"#fff1f2"},B:{bg:"linear-gradient(135deg, rgba(251,191,36,0.94), rgba(180,83,9,0.95))",border:"rgba(253,224,71,0.98)",glow:"rgba(251,191,36,0.82)",text:"#fffbea"},N:{bg:"linear-gradient(135deg, rgba(163,230,53,0.92), rgba(63,98,18,0.95))",border:"rgba(217,249,157,0.95)",glow:"rgba(163,230,53,0.78)",text:"#f7fee7"},S:{bg:"linear-gradient(135deg, rgba(192,132,252,0.92), rgba(107,33,168,0.95))",border:"rgba(233,213,255,0.95)",glow:"rgba(192,132,252,0.8)",text:"#faf5ff"},E:{bg:"linear-gradient(135deg, rgba(248,113,113,0.92), rgba(153,27,27,0.95))",border:"rgba(254,202,202,0.95)",glow:"rgba(248,113,113,0.8)",text:"#fef2f2"},X:{bg:"linear-gradient(135deg, rgba(239,68,68,0.98), rgba(127,29,29,0.98))",border:"rgba(254,226,226,1)",glow:"rgba(239,68,68,0.95)",text:"#fff7f7"},DEFAULT:{bg:"linear-gradient(135deg, rgba(51,65,85,0.92), rgba(15,23,42,0.95))",border:"rgba(148,163,184,0.85)",glow:"rgba(148,163,184,0.45)",text:"#f8fafc"}},JQ=(j,W)=>{if(!W)return 0;return(j+W)%W},XZ=(j,W,V=!1)=>{const T=Array.isArray(W)?W.length:0;if(!T)return null;if(T===1)return 0;if(V){const L=W.map((b,f)=>({item:b,idx:f})).filter(({item:b,idx:f})=>f!==j&&b&&b.url).map(({idx:b})=>b);if(!L.length)return JQ((j??0)+1,T);return L[Math.floor(Math.random()*L.length)]}return JQ((j??0)+1,T)},dJ=(j)=>ZX[j]||ZX.DEFAULT,$X=(j,W=!1)=>{const V=j==="INICIAL"?ZX.DEFAULT:j==="100"?ZX.B:dJ(j);return{"--btn-neon-color":V.glow,borderColor:V.border,color:V.text,boxShadow:W?`inset 0 1px 0 rgba(186,230,253,0.35), 0 0 0 1px ${V.border}, 0 0 20px ${V.glow}`:`inset 0 1px 0 rgba(186,230,253,0.2), 0 0 0 1px color-mix(in srgb, ${V.border} 65%, rgba(15,23,42,0.65) 35%), 0 0 12px color-mix(in srgb, ${V.glow} 60%, transparent)`,textShadow:`0 0 8px color-mix(in srgb, ${V.glow} 70%, transparent)`,filter:W?"brightness(1.14)":"brightness(1)"}},YZ=/\.(mp4|webm|ogg|mov|m4v)(\?.*)?$/i,k0=/\.(mp3|wav|ogg|m4a|aac|flac)(\?.*)?$/i,G0=/\.gif(\?.*)?$/i,S0=/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i,x0=/vimeo\.com\/(?:video\/)?(\d+)/i,QQ=(j)=>{const W=(j||"").trim();if(!W)return null;const V=W.match(S0);if(V)return{provider:"youtube",src:`https://www.youtube.com/embed/${V[1]}`,title:"Video de YouTube"};const T=W.match(x0);if(T)return{provider:"vimeo",src:`https://player.vimeo.com/video/${T[1]}`,title:"Video de Vimeo"};return null},P0=["eporner.com"],ZZ=(j="")=>{const W=String(j||"").trim();if(!W||W.startsWith("data:")||W.startsWith("blob:"))return!1;try{const T=new URL(W).hostname.toLowerCase();return P0.some((L)=>T===L||T.endsWith(`.${L}`))}catch{return!1}},HJ=(j="",W="")=>{const V=String(j||"").trim();if(!V||ZZ(V))return W;return V},nJ=(j="",W="")=>{if(W==="video"||W==="image")return W;const V=(j||"").trim();if(YZ.test(V)||QQ(V))return"video";return"image"},uX=(j="")=>{return typeof j==="string"?j.trim():""},n=(j,W="")=>{if(typeof j==="string")return{url:HJ(j.trim(),""),label:"",type:nJ(j,W),autor:""};if(j&&typeof j==="object"){const V=HJ((j.url||"").trim(),"");return{url:V,label:JJ.includes(j.label)?j.label:"",type:nJ(V,j.type||W),autor:uX(j.autor)}}return{url:"",label:"",type:nJ("",W),autor:""}},b0=(j)=>n(j).url,$Z=(j)=>n(j).label,pJ=(j)=>n(j).type,fX=(j,W,V="image")=>Array.isArray(j?.[W])?j[W].map((T)=>n(T,V)).filter((T)=>T.url):[],v0=(j={})=>({firebaseId:NJ,nombre:"Anónimo",nacionalidad:"",ciudad:"",profesion:"Galería",fechaNacimiento:"",estaturaCm:"",fotos:[],galeria:{fotos:fX(j,"fotos","image"),gifs:fX(j,"gifs","image"),videos:fX(j,"videos","video")},batallaFotosPreferidas:mX(),puntuaciones:MQ(),isAnonymousGallery:!0}),I0=(j={})=>{if(j?.isAnonymousGallery||j?.firebaseId===NJ)return"anonimo";return j?.firebaseId||""},g0=(j={})=>{const W=HJ(String(j?.fotos?.[0]||"").trim(),""),V=qJ(j?.batallaFotosPreferidas||j?.galeria?.battlePhotoPreferences||{});if(!W)return!1;return sJ.filter((T)=>T.id!=="perfil").every((T)=>Boolean(String(V?.[T.id]||"").trim()))},h0=(j={})=>{const W=[],V=HJ(String(j?.fotos?.[0]||"").trim(),""),T=qJ(j?.batallaFotosPreferidas||j?.galeria?.battlePhotoPreferences||{});if(!V)W.push("Perfil");sJ.filter((L)=>L.id!=="perfil").forEach((L)=>{if(!String(T?.[L.id]||"").trim())W.push(L.label)});return W},y0=(j=null)=>{if(!j)return;const W=window.open("","_blank");if(!W)return;const V=j?.firebaseId||j?.id||"",T=Array.isArray(j?.galeria?.fotos)?j.galeria.fotos.map((R)=>n(R,"image")).filter((R)=>R.url).map((R,N)=>({...R,sourceTag:"fotos",sourceIndex:N})):[],L=T.filter((R)=>R.type==="image"&&R.sourceTag==="fotos"),b=qJ(j?.batallaFotosPreferidas||j?.galeria?.battlePhotoPreferences||{}),f=HJ(String(j?.fotos?.[0]||j?.foto||"").trim(),""),QJ=T.filter((R)=>R.type==="image"&&R.url).reduce((R,N)=>{R[N.url]=N;return R},{}),c=T.length?T.map((R,N)=>`
                    <button
                        type="button"
                    ...
                    ${sJ.map((U)=>{const R=U.id==="perfil",N=R?c:QJ[U.id]||"",G=Boolean(N),z=!R;return`
                            <div class="gallery-slot-card" data-slot-id="${U.id}" style="border:1px solid ${G?"rgba(74,222,128,0.95)":"rgba(248,113,113,0.95)"}; border-radius:10px; padding:10px; background:${G?"linear-gradient(155deg, rgba(20,83,45,0.95) 0%, rgba(21,128,61,0.82) 45%, rgba(5,46,22,0.96) 100%)":"linear-gradient(155deg, rgba(127,29,29,0.96) 0%, rgba(185,28,28,0.84) 45%, rgba(69,10,10,0.96) 100%)"}; box-shadow: inset 0 2px 0 rgba(255,255,255,0.24), inset 0 -2px 0 rgba(2,6,23,0.45), inset 0 0 0 1px ${G?"rgba(187,247,208,0.25)":"rgba(254,202,202,0.22)"}, 0 8px 16px rgba(2,6,23,0.45), 0 0 18px ${G?"rgba(74,222,128,0.35)":"rgba(248,113,113,0.3)"}; position: relative;">
                                ...
                                <div style="margin-top:8px; border-radius:8px; overflow:hidden; aspect-ratio:4/3; border:1px dashed rgba(148,163,184,0.45); background: rgba(2,6,23,0.86); display:flex; align-items:center; justify-content:center;">
                                    ${G?`<img src="${N}" alt="Vista previa ${U.label}" onerror="${cX}" style="width:100%; height:100%; object-fit:cover; display:block;" />`:'<span style="font-size:10px; color:#fca5a5; text-transform:uppercase; letter-spacing:0.08em; font-weight:700; text-align:center; padding:0 8px;">Sin foto designada</span>'}
                                </div>
