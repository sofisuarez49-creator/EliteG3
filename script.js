(async () => {
        const getFirebaseDb = async () => {
            const existingDb = await (window.firebaseDbReady || Promise.resolve(window.firebaseDb));
            if (existingDb) return existingDb;

            const [{ initializeApp, getApps, getApp }, { getDatabase, ref, onValue, off, get, set, update, remove, push }] = await Promise.all([
            ...
            const createRefCompat = (nativeDb, path) => {
                const queryRef = ref(nativeDb, path);
                ...
                return {
                    ...
                    set: (value) => set(queryRef, value),
                    update: (value) => update(queryRef, value),
                    remove: () => remove(queryRef),
                    push: (value) => {
            ...
            const nativeDb = getDatabase(app);
            const dbCompat = { ref: (path) => createRefCompat(nativeDb, path) };
